...
const phaseCourante =
  phases.find((p) => taches.some((t) => t.phase === p && !LIVREE.has(t.statut))) ?? phases.at(-1)!;
...
const RANGS: { etat: string; rang: number; bloque: string }[] = [
  ...
  { etat: 'UNSTABLE', rang: 2, bloque: 'des contrôles encore en cours' },
  {
    ...
    rang: 3,
    bloque:
      'en retard sur `main` — `gh pr update-branch` AU MOMENT de prendre le créneau, pas avant',
  },
...
const rangDe = (p: PrOuverte): { rang: number; bloque: string } => {
  ...
  return r
    ? { rang: r.rang, bloque: r.bloque }
    : { rang: 6, bloque: `état \`${p.mergeStateStatus}\` — à qualifier à la main` };
};
/**
...
const forge = (() => {
  const figee =
    CHEMIN_FORGE === null
      ? null
      : (JSON.parse(readFileSync(CHEMIN_FORGE, 'utf8')) as {
          prs: PrOuverte[];
          issues: string;
          main: { sha: string; date: string };
        });
  const prs: PrOuverte[] = figee
    ? figee.prs
    : (() => {
        const brut = sh('gh', [
          'pr',
          'list',
          '--json',
          'number,headRefName,mergeStateStatus,isDraft,title',
          '--limit',
          '50',
        ]);
        try {
          return JSON.parse(brut || '[]') as PrOuverte[];
        } catch {
          return [];
        }
      })();
   ...
   */
  const brutIssues = figee
    ? figee.issues
    : sh('gh', ['issue', 'list', '--state', 'open', '--json', 'number,labels', '--limit', '200']);
  ...
  if (githubLu) {
    try {
      ...
      /* réponse illisible : traitée comme une lecture vide, et dite comme telle plus bas */
    }
  }

  const shaMain = figee ? figee.main.sha : sh('git', ['rev-parse', '--short', 'origin/main']);
  const dateMain = figee
    ? figee.main.date
    : sh('git', ['log', '-1', '--format=%cI', 'origin/main']);
...
lignes.push(
  `${faitPhase.length}/${faitPhase.length + restant.length} tâches terminées · reste ${restant.reduce((s, t) => s + t.estimateDays, 0).toFixed(2)} j estimés.`
);
lignes.push('');
...
if (!questions.length) {
  ...
  if (questions.length > PLAFOND_QUESTIONS) {
    lignes.push('');
    lignes.push(
      `🛑 **${questions.length} questions ouvertes, plafond ${PLAFOND_QUESTIONS} : l'autopilote s'arrête.** Il faut un arbitrage avant de composer un nouveau lot.`
    );
  }
}
lignes.push('');
...
lignes.push('');
// ⚠️ UNE LIGNE ÉMISE DANS UNE BRANCHE QUI LIT LA FORGE EST ELLE-MÊME NOURRIE PAR LA FORGE — ET
// C'EST LE TÉMOIN « exemption portante » QUI L'A APPRIS À GOV-090.
//
// Ces prose-ci ne contiennent aucune valeur de la forge, et on les avait donc laissées COMPARÉES.
// Faux : c'est leur PRÉSENCE qui dépend de la forge. `forge.file()` décide de la branche, donc
// sous une forge sans PR ouverte la vue porte « Aucune PR ouverte », et sous une forge qui en a
// elle porte le tableau. Une vue rendue sous A et jugée sous B rougissait. La lecture est donc
// RAPPELÉE dans chaque ligne que la branche émet, en-têtes de tableau compris.
//
// ⚠️ ET LA CONCLUSION QU'ON EN AVAIT TIRÉE ÉTAIT TROP LARGE — c'est la lentille `mutation` qui
// l'a mesuré : « dans cette rubrique, TOUT dépend de la forge » était vrai de ce que les deux
// branches écrivaient, pas de ce qu'elles DEVAIENT écrire. La doctrine d'ordre (RM-09) ne dépend
// d'aucune PR : elle vivait dans la branche par accident de rédaction, et s'y trouvait donc
// exemptée par sa PRÉSENCE — réécrivable à la main sans un rouge, sur la rubrique que A04 lit
// pour choisir la fusion suivante, et sans même figurer parmi les rubriques « non converties ».
// Le remède n'est pas d'assouplir la règle d'exemption : c'est de SORTIR la doctrine de la
// branche. Une prose invariante s'écrit hors du `if` ; elle devient comparée par construction.
if (!forge.file().length) {
  pousser(() => {
    forge.file();
    return 'Aucune PR ouverte.';
  });
} else {
  pousser(() => {
    forge.file();
    return '| # | PR | Branche | Ce qui la bloque |';
  });
  pousser(() => {
    forge.file();
    return '| --- | --- | --- | --- |';
...
lignes.push('');
// ÉMISE SANS CONDITION, SOUS LES DEUX BRANCHES : ne lit rien de la forge, donc COMPARÉE.
pousser(
  () =>
...
}
{
  ...
  // ⚠️ BRANCHE NOURRIE PAR LA FORGE : `enVol` dérive de `revendiqueursDe`, qui lit
  // `forge.revendications()`. La PRÉSENCE de ces lignes dépend donc de la forge, même quand leur
  // texte n'en porte aucune valeur — la lecture est rappelée dans chacune.
  if (!enVol.length) {
    pousser(() => {
      forge.revendications();
      return 'Aucune tâche revendiquée. Un agent ne prend jamais une tâche non revendiquée (REQ-GOV-007) : la revendication passe par l’orchestrateur.';
    });
  } else {
    pousser(() => {
      forge.revendications();
      return '| Tâche | Revendiquée par | Issue | Statut |';
    });
    pousser(() => {
      forge.revendications();
      return '| --- | --- | --- | --- |';
  ...
  const perimees = taches.filter(
    (t) => LIVREE.has(t.statut) && revendiqueursDe(t).length > 0 && !t.owner
  );
  ...
  if (perimees.length > 0) {
    // `perimees` dérive AUSSI de `revendiqueursDe` : la présence de cette ligne dépend de la
    // forge, même si son texte ne lit que `docs/tasks.json`.
    pousser(() => {
      forge.revendications();
      return `⚠️ ${perimees.length} tâche(s) livrée(s) sans \`owner\` consolidé dans \`docs/tasks.json\`.`;
    });
    lignes.push('');
  }
}
...
lignes.push('');
{
  ...
  // RUBRIQUE CONVERTIE (GOV-090, second tour) : chaque ligne passe par `pousser`, et la lecture
  // de la forge est RAPPELÉE dans celles qui l'affichent. La population du jour dépend de
  // `forge.dateMain()` par sa valeur ET par sa présence ; la doctrine, elle, n'en dépend pas et
  // sort donc de la branche — c'est la seule façon de la faire comparer.
  if (!decisionsDuJour.length) {
    pousser(() => {
      forge.dateMain();
      return `Aucun ADR daté du ${jourMain || '?'} (jour du dernier atterrissage).`;
    });
  } else {
    // Une seule ligne, sans puce : une zone exemptée n'ouvre ni liste ni citation (`horsDeSaZone`).
    pousser(() => {
      forge.dateMain();
      return decisionsDuJour.map((a) => `\`docs/adr/${a.fichier}\` — ${a.titre}`).join(' · ');
    });
  }
  lignes.push('');
  // ÉMISE SANS CONDITION : ne lit rien de la forge, donc COMPARÉE octet par octet.
  pousser(
    () =>
...
lignes.push('');
{
  ...
  //
  // ⚠️ C'EST LA RUBRIQUE QUE A04 LIT POUR CHOISIR LA FUSION SUIVANTE (RM-09), et elle était
  // exemptée EN BLOC parce qu'UNE de ses lignes nomme la PR en tête de file. Les deux autres ne
  // lisent que `docs/tasks.json` : elles sont désormais émises hors de toute branche que la forge
  // décide, donc COMPARÉES. Ce qui reste libre est exactement le NUMÉRO de la PR prête — une
  // valeur de la forge, que la comparer reviendrait à mesurer. `partners/ADR-0019` le nomme.
  if (prete) {
    pousser(() => {
      forge.file();
  ...
  lignes.push('');
  // ÉMISE SANS CONDITION : la DOCTRINE du pas suivant, comparée octet par octet.
  pousser(
    () =>
...
pousser(
  () =>
    `\`origin/main\` = \`${forge.shaMain() || '?'}\` (${forge.dateMain() || '?'}). Vérifier \`x-partners-build-sha\` avant toute nouvelle fusion.`
);
lignes.push('');
// ÉMISE SANS CONDITION : ne lit rien de la forge, donc COMPARÉE octet par octet.
pousser(
...
// rubrique plus bas ; aucune n'est saisie. Un résumé tenu à la main ment au premier oubli.
{
  ...
  const bloc = [
    ...
    ligneDeReprise(() => '| --- | --- |'),
    ligneDeReprise(
      () => `| Où est \`main\` ? | \`${forge.shaMain() || '?'}\` — ${forge.dateMain() || '?'} |`
    ),
    ligneDeReprise(() => {
...
function horsDeSaZone(ligne: string): string | null {
  ...
  if (!DEBUT_PERMIS.test(ligne))
    return `commence par ${JSON.stringify(ligne.slice(0, 3))}, hors des débuts permis — au rendu, une construction qui peut sortir de sa zone`;
  return null;
}

/** La question d'une ligne du bloc de reprise (sa première cellule), ou `null` si la ligne est une prose. */
export const question = (l: string): string | null =>
  l.startsWith('|') ? (l.split('|')[1] ?? '').trim() : null;
/** Les proses du bloc de reprise, dans l'ordre : toute ligne non vide qui n'est pas une question. */
export const proses = (corps: string): string[] =>
...
const LECTURES: readonly (readonly [RegExp, ...string[]])[] = [
  [
    ...
    'tâches terminées en phase courante',
    'tâches de la phase courante',
    'jours restants en phase courante',
  ],
  [
    /^\| Où en est la phase \? \| phase (-?\d+) — (\d+)\/(\d+) tâches, reste ([\d.]+) j \|$/m,
    'phase courante (bloc de reprise)',
    'tâches terminées (bloc de reprise)',
    'tâches de la phase (bloc de reprise)',
    'jours restants (bloc de reprise)',
  ],
  [
    /^\| Ce qui bloque \| (\d+) tâche\(s\) bloquée\(s\) ou en attente externe · (\d+) question\(s\) pour Will \|$/m,
    'tâches bloquées ou en attente (bloc de reprise)',
    'questions ouvertes pour Will (bloc de reprise)',
  ],
  [
    /^\*\*([\d.]+) j\*\* sur (\d+) taches enchainees/m,
    'jours du chemin critique',
...
function mesures(texte: string): Map<string, string> {
  ...
  for (const [re, ...noms] of LECTURES) {
    ...
    if (r)
      noms.forEach((n, k) => {
        if (r[k + 1] !== undefined) m.set(n, r[k + 1]!);
      });
  }
...
function premiereDifference(attendu: string, trouve: string): string {
  ...
  for (let i = 0; i < Math.max(a.length, t.length); i++) {
    if (a[i] !== t[i]) {
      const court = (s: string | undefined) =>
        s === undefined ? '(ligne absente)' : `« ${s.slice(0, 160)} »`;
...
function comparer(
  ...
  /**
   * LE GAIN DE GOV-090, COMPTÉ SUR SA POPULATION — ET LES DEUX NOMBRES SORTENT ENSEMBLE.
   *
   * `comparees` : les lignes non vides d'une rubrique exemptée que le vérificateur confronte
   * réellement. `total` : toutes les lignes non vides des rubriques exemptées, converties ou non.
   * Le premier tour de GOV-090 n'imprimait que le numérateur — seul des quatre compteurs de ce
   * vert à ne pas porter sa population. « 1 ligne comparée » se lit comme un succès alors qu'il ne
   * dit rien du reste : un compteur sans dénominateur ment par omission. Le rendre ne coûte rien,
   * et ne pas le rendre a coûté un refus de lentille.
   *
   * ⚠️ CE DÉNOMINATEUR N'EST PAS UNE CONSTANTE et ne se recopie dans aucun document : « File de
   * fusion » écrit une ligne par PR ouverte, donc la population bouge avec la forge. C'est un
   * nombre qu'on IMPRIME, jamais un nombre qu'on tient à jour.
   */
  const lignesDansExemptees = { comparees: 0, total: 0 };
  ...
  const contenir = (zone: string, texte: string) => {
    texte.split('\n').forEach((l, k) => {
      ...
      if (s !== null) {
        ecarts.push({
          famille: 'structure_dans_une_exemption',
          message: `${zone}, ligne ${k + 1} de la vue sur le disque : « ${l.slice(0, 80)} » ${s}. Son CONTENU n'est pas comparé ; ce qu'elle fait au reste du document l'est.`,
        });
      }
    });
  };
  ...
  for (const [nom, valeur] of mA) {
    ...
    if (vu === undefined)
      ecarts.push({
        famille: 'mesure_absente',
        message: `${nom} : introuvable dans la vue sur le disque, ses sources en produisent ${valeur}`,
      });
    else if (vu !== valeur)
      ecarts.push({
        famille: 'vue_perimee',
        message: `${nom} : la vue sur le disque dit ${vu}, ses sources produisent ${valeur}`,
      });
  }
  ...
  for (const [ou, liste] of [
    ['ce que produisent les sources', rA],
    ['la vue sur le disque', rD],
  ] as [string, Rubrique[]][]) {
    const vus = new Set<string>();
    for (const r of liste) {
      if (vus.has(r.titre)) {
        ecarts.push({
          famille: 'rubrique_dupliquee',
          message: `rubrique « ${r.titre} » dans ${ou} : elle apparaît deux fois — laquelle fait foi ? Aucune : corrige la source.`,
        });
        continue;
      }
      vus.add(r.titre);
  ...
  for (const t of titresA)
    if (!titresD.includes(t))
      ecarts.push({
        ...
        message: `rubrique « ${t} » : absente de la vue sur le disque, produite par ses sources`,
      });
  for (const t of titresD)
    if (!titresA.includes(t))
      ecarts.push({
        famille: 'rubrique_en_trop',
        message: `rubrique « ${t} » : présente dans la vue sur le disque, produite par aucune source`,
      });
  ...
  for (const r of rA) {
    ...
    if (motif !== null) {
      ...
      // LA POPULATION EST COMPTÉE AVANT TOUTE CONDITION : c'est elle le dénominateur, et il ne
      ...
      if (surPlace) {
        ...
        if (!convertie) {
          ...
          // ⚠️ ON COMPARE PAR PRÉSENCE, JAMAIS PAR POSITION — ET C'EST UN TÉMOIN QUI L'A APPRIS.
          //
          // Première écriture : alignement par INDEX, plus un refus quand les deux corps n'ont
          // pas le même nombre de lignes. Le témoin « exemption portante » de
          // `vues-derivees.spec.ts` — qui rend la vue sous DEUX forges — l'a fait rougir :
          // « File de fusion » portait 8 lignes sous une forge et 3 sous l'autre. Évidemment :
          // une ligne PAR PR ouverte. Dans ces rubriques, le NOMBRE de lignes est lui-même une
          // valeur de la forge. La position d'une ligne de doctrine l'est donc aussi.
          //
          // Une ligne qui ne lit rien de la forge est INVARIANTE : elle doit Être PRÉSENTE dans
          // la rubrique du disque, où qu'elle soit. C'est ce qui est exigible sans rien supposer
          // de ce que la forge insère autour.
          for (const ligne of lA) {
            if (ligne === '') continue;
    ...
    ecarts.push({
      famille: 'vue_perimee',
      message: `rubrique « ${r.titre} » : ${premiereDifference(r.corps, surPlace.corps)}`,
    });
  }
  ...
  if (blocA && blocD) {
    ...
    for (let k = 0; k < Math.max(fA.length, fD.length); k += 1) {
      if (fA[k] !== fD[k]) {
        ecarts.push({
          famille: 'vue_perimee',
          message: `bloc de reprise, ligne ${k + 1} : la vue sur le disque porte ${fD[k] ?? '(rien)'}, ses sources produisent ${fA[k] ?? '(rien)'} — la FORME du bloc est dérivée, elle ne se réarrange pas à la main`,
        });
        break;
      }
    }
    ...
    const indexer = (corps: string, ou: string) => {
      ...
      for (const l of corps.split('\n')) {
        ...
        if (m.has(q)) {
          ecarts.push({
            famille: 'ligne_de_reprise_dupliquee',
            message: `bloc de reprise, ${ou} : la question « ${q} » apparaît deux fois — laquelle fait foi ? Aucune : corrige la source.`,
          });
          continue;
        }
        m.set(q, l);
    ...
    for (const q of new Set([...qA.keys(), ...qD.keys()])) {
      ...
      if (attendue === undefined) {
        ecarts.push({
          famille: 'vue_perimee',
          message: `bloc de reprise : la ligne « ${q} » est sur le disque et n'est produite par AUCUNE source`,
        });
        continue;
      }
      if (vue === undefined) {
        ecarts.push({
          famille: 'vue_perimee',
          message: `bloc de reprise : la ligne « ${q} » est produite par les sources et ABSENTE de la vue sur le disque`,
        });
        continue;
      }
      ...
      if (vue !== attendue)
        ecarts.push({
          famille: 'vue_perimee',
          message: `bloc de reprise, ligne « ${q} » : la vue sur le disque dit « ${vue.slice(0, 160)} », ses sources produisent « ${attendue.slice(0, 160)} »`,
        });
    }
    ...
    for (let k = 0; k < Math.max(pA.length, pD.length); k += 1) {
      ...
      if (a === undefined) {
        ecarts.push({
          famille: 'vue_perimee',
          message: `bloc de reprise : la prose n°${k + 1} « ${d!.slice(0, 60)} » est sur le disque et n'est produite par AUCUNE source`,
        });
        continue;
      }
      if (d === undefined) {
        ecarts.push({
          famille: 'vue_perimee',
          message: `bloc de reprise : la prose n°${k + 1} « ${a.slice(0, 60)} » est produite par les sources et ABSENTE de la vue sur le disque`,
        });
        continue;
      }
      ...
      if (a !== d)
        ecarts.push({
          famille: 'vue_perimee',
          message: `bloc de reprise, prose n°${k + 1} « ${a.slice(0, 60)} » : la vue sur le disque et ses sources diffèrent`,
        });
    }
  }
...
if (!LANCE_EN_SCRIPT) {
  ...
  console.log(
    `PLAN-STATE régénéré — phase ${phaseCourante}, ${enCours.length} en cours, ${questions.length} question(s) ouverte(s).`
  );
