...
const phaseCourante =
  phases.find((p) => taches.some((t) => t.phase === p && !TERMINES.has(t.statut))) ??
  phases.at(-1)!;
...
}
{
  ...
  const perimees = taches.filter(
    (t) => TERMINES.has(t.statut) && revendiqueursDe(t).length > 0 && !t.owner
  );
...
lignes.push('');
{
  ...
  if (prete) {
    lignes.push(
      `1. **Fusionner #${prete.number}** — elle est en tête de file et ne bloque sur rien. Lire \`mergeStateStatus\` et fusionner dans le MÊME appel (RM-09), puis vérifier l’atterrissage.`
    );
  }
  if (suivante) {
    lignes.push(
...
lignes.push(
  `\`origin/main\` = \`${shaMain || '?'}\` (${dateMain || '?'}). Vérifier \`x-partners-build-sha\` avant toute nouvelle fusion.`
);
lignes.push('');
lignes.push(
