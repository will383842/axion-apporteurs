      ...
      if (orphelins.length > 0) {
        ajouter(
          ...
          `La PR modifie ${orphelins.length} fichier(s) de code qu'aucune de ses tâches ne déclare : ` +
            ...
            `\`ajouter-path.mjs\` : déclare les AUTRES tâches du lot dans le champ \`Lot:\` du corps ` +
            `(GOV-096, mesuré sur la PR 114 — ajouter leurs chemins à la tâche du titre écrirait ` +
            `qu'elle touche des fichiers qui appartiennent à d'autres tâches, et la disjonction ` +
            `des lots se calcule sur ces mêmes \`paths\`). Sinon : ajoute le chemin à la tâche par ` +
            `\`outils/ajouter-path.mjs\`, ou sors le fichier du périmètre de cette PR — un fichier ` +
            `écrit hors de ce qu'on a déclaré écrire, c'est un lot dont la disjonction ne veut ` +
            `plus rien dire.`
        );
      }
    }
