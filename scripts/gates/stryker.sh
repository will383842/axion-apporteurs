...
#   2. `scripts/mutation/rapport.ts` relit le rapport et le seuil DÉCLARÉ, et NOMME chaque mutant non
...
# sa propre configuration) ; le lecteur est pris dans `scripts/mutation/`, relativement à ce script.
set -uo pipefail
...
node --import tsx "$ICI/../mutation/rapport.ts"
CODE_RAPPORT=$?
