...
| `gov:entite` | CPL-T01 | `scripts/gates/gov-entite.ts` | — | pnpm gov:entite:prove — 12 familles, un temoin chacune sur un univers INJECTE, dont secret_commite (un IBAN commite dans un fichier suivi), valeur_recopiee (un SIREN du registre recopie ailleurs) et contenu_illisible (un temoin par cause : l octet NUL d un texte UTF-16, la sequence UTF-8 invalide d un texte Latin-1) ; 9 contre-temoins restent verts, dont l univers conforme et un texte UTF-8 a marque d ordre et ideogrammes |
