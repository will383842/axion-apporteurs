...
| `plan-state:verifier` | GOV-035 | `scripts/plan-state/build.ts` | — | tests/unit/gouvernance/vues-derivees.spec.ts, bloc « GOV-035 » : 25 temoins verts le 2026-09-14, dont 2 contre-temoins nommes (vue fraichement rendue, vue commitee du depot) et le temoin « exemption PORTANTE », qui rend la vue sous deux forges figees. ROUGE mesure AVANT le code : « expected 'PLAN-STATE regenere — phase -1, 0 en ...' to contain '✅' » — `--verifier` etait ignore et le script REECRIVAIT le fichier qu'on lui demandait d'inspecter. Rejoue apres le code sur la vue falsifiee a trois compteurs, ancres conservees : exit 1, 8 ecarts nommes en taches et en jours. Le dernier temoin du bloc ne mesure pas la gate : il confronte `FAMILLES` aux familles vues dans la SORTIE des autres temoins, dans les deux sens, et exige qu'aucune sortie verte n'en porte. La valeur precedente de ce champ annoncait « 10 temoins verts le 2026-09-12 » : elle datait d'avant trois tours de reprise, qui en avaient porte le compte a 22. |
