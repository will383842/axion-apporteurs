...
| 0 | Fondations, sécurité, charte | 42 | 11 | 31 |
...
| `gov:entite` | CPL-T01 | `scripts/gates/gov-entite.ts` | — | pnpm gov:entite:prove — 14 familles, un temoin chacune sur un univers INJECTE, dont aucune_reference_publique (les trois identifiants publics a la sentinelle — l etat d une decision ROUVERTE, donc legitime — et un fichier de code qui porte le SIREN du registre : la garde REFUSE au lieu de rendre un vert silencieux) et secret_commite (un IBAN commite dans un fichier suivi), valeur_recopiee (un SIREN du registre recopie ailleurs, et un second temoin qui le recopie DANS UNE FIXTURE — nos numeros n ont rien a y faire non plus) et contenu_illisible (un temoin par cause : l octet NUL d un texte UTF-16, la sequence UTF-8 invalide d un texte Latin-1) et contenu_publie_non_lu (un temoin par cause : un pointeur Git LFS, un fichier que l attribut filter confie a LFS) ; 12 contre-temoins restent verts, dont l univers conforme, un texte UTF-8 a marque d ordre et ideogrammes, une documentation qui cite la premiere ligne d un pointeur Git LFS, et LES DEUX FACES du regime des numeros publics : le SIREN et la TVA d un TIERS restent verts dans un fichier de CODE puis dans une FIXTURE, la ou les notres rougissent aux deux endroits |
...
| `G-SEC-AST-PRISMA` | QA-T07 | `scripts/gates/semgrep.ts` | `sec:semgrep` | pnpm sec:semgrep:prove — 57 temoins rouges nommes par leur regle (dont 38 sous nosemgrep), 16 contre-temoins muets ; la spec tests/unit/qualite/semgrep-regles-maison.spec.ts voit la sortie NON NULLE du binaire sur une copie des regles portant une regle orpheline. ROUGE d origine de QA-T07, sur 87fb212, executeur et spec ecrits AVANT .semgrep.yml : `pnpm vitest run tests/unit/qualite/semgrep-regles-maison.spec.ts` rend `Tests 81 failed \| 4 passed (85)`, premier echec `[regles_introuvables] Le fichier de regles maison ... .semgrep.yml est introuvable : aucune regle maison ne peut mordre.: expected null not to be null` ; puis .semgrep.yml avec la SEULE regle SQL : `Tests 46 failed \| 29 passed \| 10 skipped (85)`, `[temoin_muet] prisma/import (src/server/espace/a/b/import.ts:1) : « axion-prisma-hors-couche-d-acces » ne mord pas. Constats sur ce fichier : aucun.` Mutants tues : --disable-nosem retire (38 temoin_muet), image :latest (image_non_epinglee), jeux publics vides (plancher_public), sans .semgrepignore vide (fichier_non_analyse), segment db retire, new PrismaClient retire, languages sans javascript, include sans (espace), regle 1 elargie a /src/** (15 faux_positif), continue-on-error sur l etape CI, etape --prove retiree. |
...
Aucune de ces **79** entrées ne porte de `preuveRouge` : personne ne les a vues rougir.
...
### Phase 0 — fondations, sécurité, charte (31)
