...
| `plan-state:verifier` | GOV-035 | `scripts/plan-state/build.ts` | — | tests/unit/gouvernance/vues-derivees.spec.ts, bloc « GOV-035 » : 26 temoins, dont 2 contre-temoins nommes (vue fraichement rendue, vue commitee du depot), le temoin « exemption PORTANTE » (deux forges figees, dont une aux valeurs hostiles) et le temoin « importer le module n'ecrit rien ». ROUGE mesure AVANT le code : « expected 'PLAN-STATE regenere — phase -1, 0 en ...' to contain '✅' » — `--verifier` etait ignore et le script REECRIVAIT le fichier qu'on lui demandait d'inspecter. Rejoue apres le code sur la vue falsifiee a trois compteurs, ancres conservees : exit 1, 8 ecarts nommes en taches et en jours. Le temoin « structure_dans_une_exemption » porte des ecritures qui atteignent le reste du document au rendu GitHub, dans les trois formes de zone exemptee, et un contre-temoin qui garde chaque debut permis. Le dernier temoin du bloc ne mesure pas la gate : il confronte `FAMILLES` aux familles vues dans la SORTIE des autres temoins — toute ligne qui commence par `[…]` — dans les deux sens, et exige qu'aucune sortie verte n'en porte. |
