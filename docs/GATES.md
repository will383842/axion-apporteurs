...
| `securite:schema-pii` | SEC-08 | `scripts/gates/schema-pii.ts` | `G-SEC-SCHEMA-PII` | pnpm securite:schema-pii:prove — 10 familles rougissent sur 32 temoins, 5 contre-temoins verts (dont les etalements conditionnels data: { ...(ip ? { ipHash: ip } : {}) } et data: { ...(e && { emailHash: e }) }, joues par securite sur la PR 126, et un temoin par mutant de la revue mutation 5321838301 : appel quelconque admis comme producteur, cle update retiree, chaque branche d'un ternaire, de &&, de \|\| et de ?? coupee, as/satisfies non deballes, producteur admis sur un Chiffre) ; vues INJECTEES (RM-11). Dix mutants de la garde rejoues : chacun fait rougir --prove ET le spec. Binaire sur le depot (SEC-08, 2026-09-25) : createdIp String? pose dans le modele Evenement -> exit 1, [colonne_personnelle_en_clair] Evenement.createdIp ; un fichier suivi src/ ecrivant data: { emailChiffre: Buffer.from(e) } -> exit 1, [chiffre_hors_primitive] nommant le champ ; depot intact -> exit 0, 8 champs, 31 fichiers, 0 site. Garde mutee (chiffre admis partout) -> le temoin a deux faces de tests/unit/securite/chiffrement-avec-aad.spec.ts rougit. Rouge constate par: A05 |
