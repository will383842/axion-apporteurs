...
| `config/exemptions-corps-publie.json` | A01 commite. ⚠️ **Aucune approbation SUPPLÉMENTAIRE n'est exigée ici** : `securite` figure dans `DEUX_PREMIERES` et juge donc DÉJÀ toute PR — l'écrire dans cette colonne promettait un contrôle qui n'existe pas | — (⚠️ **et ce `—` ne veut PAS dire « aucun agent ne peut écrire » comme pour la ligne précédente : A01 commite ce fichier. Il veut dire qu'AUCUN label ne l'arme, et c'est une lacune, pas une protection** — voir `partners/ADR-0010` §Reste à faire) | `partners/ADR-0010` ; `.github/CODEOWNERS` — **le seul fichier du dépôt qui puisse ABSOUDRE un rouge bloquant** : une ligne y transforme un échec de Gate A en vert. `config/entite.json` ne peut que CONTRAINDRE ; celui-ci absout. Le label est celui du poste qui commite, comme les autres registres ; l'approbation bloquante est portée par le protocole de revue et par CODEOWNERS, ce dépôt n'ayant pas de label de rôle pour cette lentille. |
