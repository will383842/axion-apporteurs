...
les tests d'intégration entrent dans `pnpm test`. L'ADR est `partners/ADR-0014`.

**Reste.** Un propriétaire qui désarme le déclencheur et recalcule toute la queue n'est pas détecté
tant que la tête n'est pas ancrée hors de la base : DM-20, et un test tient cette limite. La chaîne
...
chaîne de hash sans secret ne détecte que l'acteur qui ne recalcule pas : écrire « détecte toute
altération » promettait ce que l'algorithme public ne peut pas tenir, et la lentille securite l'a
démontré en base réelle.
