...
Après deux refus de lentille le 2026-09-18, la spécification lit les titres par LA lecture de
`gov:trace`, sortie dans `scripts/lot/titres-ecrits.ts`, et la garde juge un marqueur comme jeton
délimité : `siren` et `signe` ne tenaient que par sous-chaîne, et rejoignent la dette.
