...
PR #99) atterrit, et le temoin de cette PR-la devient MUET a la fusion de celle-ci — il lit
`paquet.scripts['gov:check']`, qui vaudra la chaine vide, et ses trois assertions passeront en
n'asserant plus rien ; la correction exacte est ecrite au corps de la PR #102, elle est due dans
#99 AVANT sa fusion. Sept verbes d'outillage hors depot prescrivent encore `pnpm gov:check` : le
perimetre d'une garde s'arrete au depot, aucun des six temoins ne peut les voir, et ils sont portes
par le dossier de reprise. Deux occurrences du nom mort restent au depot et c'est voulu —
`docs/GLOSSAIRE.md` ligne 211 et `docs/REPRISE-SESSION.md` ligne 172 RACONTENT une mesure passee.
Les trois clauses non tenues de GOV-000 ne sont pas rouvertes ici.
...
collision apres coup. Plus mordant : la PR #100, fusionnee la premiere, avait ecrit dans `ci.yml`,
`scripts/reprise.ts` et `docs/adr/0011-*` que « `partners/ADR-0017` a tranche l'homonymie », en
anticipant une fusion qui n'a pas eu lieu dans cet ordre. Depuis `53cf630`, ces trois phrases de
`main` designaient le mauvais ADR, et AUCUNE garde ne pouvait le voir : `gov:adr` verifie qu'un
renvoi RESOUT vers un fichier existant, pas qu'il resout vers le BON. Un renvoi d'ADR ecrit par
anticipation est vert tant que le numero existe, quel que soit ce qu'il designe. La lecon generale :
citer un identifiant qu'une AUTRE branche n'a pas encore pose, c'est parier sur l'ordre de fusion,
et l'outillage ne garde pas ce pari.
