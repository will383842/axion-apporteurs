...
chaîné en `sha256-jcs-v1` à partir d'une genèse que la migration insère. Deux déclencheurs de base
refusent `UPDATE`, `DELETE` et `TRUNCATE`, et le verrou consultatif empêche la bifurcation. La
charge est fermée par type et jugée par la garde `journal:sans-pii`, qui s'appuie sur le lexique
unique des champs de personne. Le harnais Postgres tourne par testcontainers, et les tests
d'intégration entrent dans `pnpm test`. L'ADR est `partners/ADR-0013`. La PR porte aussi la
clôture de QA-T01 au registre (PR 59, `8c70f52`) et les revendications de DM-01, SEC-01 et SEC-02.

**Reste.** Les sept valeurs neuves d'enum attendent leur entrée au glossaire par le gardien-spec :
`docs/GLOSSAIRE.md` est réservé, aucun verbe ne l'écrit, et `partners:schema:enums` rougit
jusque-là. `schema: true` n'a pas pu être écrit, car `reecrire-champ` refuse ce champ. DM-06 rendra
`ajouterEvenement` refusant `journal_ouvert`. Ancrage de la tête hors base et vérification
périodique : DM-20. Séparation des rôles Postgres : déploiement.
...
`tests/integration/journal.spec.ts` était compté « NON » exécuté alors que vitest l'exécutait.
Aucune garde ne l'a nommé : la vue de traçabilité a divergé, sur la mauvaise cause apparente.
