...
## PR #100 — 2026-09-22 — fix(GOV-059): la porte A ne tourne plus sur une PR fusionnee, gov:pr nomme la bonne cause, et ADR 0017

**Fait.** Deux livrables, et le second mordait à chaque fusion. `partners/ADR-0017` consigne la
décision de la PR 89 : la vérité d'une revendication a quitté un fichier versionné, relu par quatre
lentilles et tenu par CODEOWNERS, pour des labels GitHub, qui n'entrent dans aucune revue — sur un
dépôt public. Il nomme la barrière réelle, ouvrir une issue est à la portée de tous et poser un
label exige `triage` ou `write`, et il la vérifie dans le code plutôt que de l'affirmer. Second
livrable, au job `gate-a` de `.github/workflows/ci.yml` : une condition sur l'état fusionné de la
PR. Le runbook impose de cocher la huitième case de la définition de terminé APRÈS la fusion, et
`ci.yml` se déclenche sur `edited` sans aucune garde sur cet état — donc à chaque PR l'édition du
corps relançait la porte A sur une tête qui n'est plus une branche, et `gov:pr` échouait à l'étape
26 sur 71 en laissant 45 étapes en sautées, dont `Lint`, `Format`, `Typecheck`, `Tests`,
`req:check` et `gov:etat`. Le message accusait à tort : il prescrivait `fetch-depth: 0` sur
`actions/checkout`, qui est déjà posé. `scripts/gates/gov-pr.ts` distingue désormais les deux
causes sur la LISIBILITÉ des deux extrémités (`git cat-file -e <sha>^{commit}`), jamais sur le
texte de l'erreur de git, qui varie selon la forme des arguments ; le message d'origine est
conservé mot pour mot là où il reste vrai, un clone `fetch-depth: 1` qui ne contient pas le sha de
base. Les témoins vivent dans `tests/unit/gouvernance/revendication-par-branche.spec.ts`, déjà
promis par le `tests{}` de GOV-059, et l'évaluateur de conditions de job est PARTAGÉ
(`tests/unit/ci/condition-de-job.ts`) parce que deux spécifications jugent la même condition pour
deux raisons différentes ; il lève sur ce qu'il ne sait pas lire, jamais de vert par ignorance.

**Reste.** Les huit cases de la définition de terminé sont vides : l'auteur ne les coche pas.
`scripts/prevol.ts` est sorti de cette branche — il appartient aux `paths` de GOV-047 depuis
l'origine et il est livré par la PR 99 sous sa propre tâche ; il avait été écrit ici par
méconnaissance du backlog. `docs/gates.json` contredit le code en trois points sur
`G-SEC-AST-PRISMA` depuis le correctif de QA-T07 : c'est la PR 82 qui l'a rendu faux, c'est elle
qui le répare, et rien n'est porté ici. Les 24 dettes relevées sur la PR 89 restent dues,
consignées hors dépôt, dont celle que trois lentilles ont nommée par trois chemins — la convention
de titre `ID` suivi d'un tiret cadratin est recopiée à la main à trois endroits. Une réserve enfin,
posée dans la section Attaque : le diff d'un sha fusionné n'est plus mesuré par la porte A, il
l'est par le run `push: main` du même sha, qui existe et qui est vert sur chaque fusion. C'est un
déplacement de garde, pas une garde en moins, et il est nommé.

**Appris.** Une liste fermée de clés de job autorisées, DÉJÀ en place, refusait la seule clé que
GitHub offre pour porter cette condition — et l'une de ses mutations témoins était précisément
cette clé. La sortie n'était pas d'élargir la liste : c'est d'y faire entrer la clé et de JUGER sa
valeur, lue dans `ci.yml` au lieu d'être retapée, sans quoi on remplaçait un refus utile par une
permission muette. Deux mutations témoins ont dû changer de forme, et c'est écrit dans le fichier :
l'analyseur YAML refuse les clés en double, donc ajouter une seconde clé déjà présente faisait
LEVER le témoin au lieu de le faire rougir sur la bonne famille, et un témoin qui lève ressemble à
un témoin qui marche. Troisième fait, contre l'intuition et vérifié par un cas du témoin plutôt que
supposé : ajouter un membre sur le NOM de l'événement n'écarterait aucun run de plus — sur `push`,
l'objet de la PR est absent, la propriété rend `null`, et GitHub compare en NOMBRES des opérandes
de types différents, si bien que la porte A tourne sur `main` comme avant ; l'écrire aurait été une
seconde écriture de la même règle. Et la condition se pose au JOB, pas aux étapes : un job sauté se
lit en une ligne, là où 71 étapes sautées une par une afficheraient exactement l'image qu'on
retire.
## PR #90 — 2026-09-22 — feat(SEC-07): frontiere axionia — 404 unique, jeton en temps constant, liste d'adresses fermee

**Fait.** `src/server/integrations/axionia/api-entrante.ts` pose la frontiere unique des API
qu'axionia appelle, et tout appel y suit le meme chemin : configuration, adresse du client, jeton
porteur, methode et route, debit, SIREN, puis lecture. Chaque refus est le meme — un 404, corps
vide, aucun en-tete, identique octet pour octet quelle qu'en soit la cause — parce qu'un 401
confirmerait que la route existe, et un 405 comme l'OPTIONS automatique de Next aussi : les deux
routes exportent donc les sept methodes, et un attrape-tout rend le meme refus a un chemin
inexistant sous le prefixe. Le jeton dedie est compare en temps constant, empreintes des deux
cotes puis comparaison sans court-circuit sur la longueur ; la liste d'adresses est fermee par
defaut, car absente, vide ou illisible personne n'entre, et elle est normalisee par le lecteur de
SEC-10, jamais par une copie. Echec ferme partout : le debit par defaut REFUSE, et la lecture
d'attribution non branchee refuse aussi — jamais « libre » par defaut, ce serait echouer ouvert
contre l'apporteur. Ce qui n'est pas dans la reponse ne peut pas fuir, le corps etant reconstruit
champ par champ depuis un schema strict, et un SIREN inconnu comme un SIREN libre rendent la meme
constante au meme instant, plancher de duree et horloge injectee : la forme ET le temps sont
indiscernables. Le journal est une ligne par appel, reconstruite depuis des champs fermes, sans le
jeton, sans l'adresse et sans message d'erreur ; 22 cas confrontes aux routes lues sur le disque,
dont 98 appels pour deux routes fois sept methodes fois sept refus. Le temoin a deux faces de
`QA-T02` cesse enfin de supposer que TOUT fichier du dossier d'integration depend du demon :
SEC-07 mesure cette supposition fausse, donc qui en depend se derive desormais de la clause
d'import vers le harnais, et la ligne attendue se construit des deux comptes, dependants en echec
et autonomes au vert. La contrainte « aucun passed » tombe, redondante et fausse, la ligne de
fichiers epinglant deja le compte exact ; l'interdiction du saut, elle, restait statique et n'a
jamais eu besoin de cette face. Un contre-temoin tient le piege : un filtre ecrit sur le TEXTE
classerait `frontiere.spec.ts` comme dependant, puisqu'il CITE le nom du temoin dans un
commentaire.

**Reste.** Le compteur de soixante par minute ne peut pas entrer au registre de `SEC-10` tant que
le texte de `REQ-SEC-012` ne le chiffre pas sous la forme que la garde de famille lit : le debit
refuse donc par defaut, et la question est ouverte au gardien du spec. La correspondance des etats
vers le vocabulaire public appartient a `INT-T07-P`, d'ou le refus de la lecture d'attribution,
que le code nomme deja. La seconde moitie de `REQ-INT-014` — cote axionia, delai de deux secondes,
cache de cinq minutes, echec ouvert avec un devis cree non verifie et une alerte, verification
rejouee par un job — appartient a `INT-T07-A` et n'est pas fermee ici, alors que la vue de
tracabilite porte desormais le temoin de cette PR en face de cette exigence. Le controle de
methode n'est exerce par aucun temoin : la forme exacte que l'en-tete dit vouloir interdire passe
au vert, apres l'authentification. Le debit de la frontiere est un port prive hors du registre des
compteurs, la comparaison en temps constant est la seule du depot et n'est mesuree par rien, un
oracle temporel leger subsiste sur l'appartenance a la liste et se mesure dans le bruit,
l'empreinte d'adresse est posee ici faute de module d'empreintes partage, et le 429 sans instant
de reprise comme la levee du limiteur n'ont aucun temoin. Enfin la gate declare une preuve rouge
nulle alors que sa fixture rouge est mesurable, la ou ses deux soeurs declarent la leur. Reste un
bruit, et il est nomme plutot que subi : la boucle sur les routes du disque fait ecrire au code de
production ses 98 lignes de journal dans la sortie de vitest, et un bruit qu'on apprend a ignorer
finit par cacher un vrai rouge. Deux issues sont ecartees pour la meme raison, qu'aucune ne vaut
son prix : doubler le flux d'erreur de l'hote violerait la charte du dossier d'integration
(`REQ-QA-006`), et poser une couture de test dans `src/` mettrait une porte derobee dans le code
livre. La reduire demanderait un puits de production configurable hors du chemin d'appel, qui
n'appartient a aucune tache ouverte.

**Appris.** Deux exigences se contredisent sur ce que la frontiere doit rendre, et la preseance
tranche en faveur de celle de l'integration : le code livre est juste, mais la PR applique une
clause et ecarte l'autre en silence — un lecteur du seul code ne peut pas savoir qu'une clause a
ete ecartee. Pire, le fichier ecrit noir sur blanc l'une des deux divergences et tait l'autre : il
apprend donc au lecteur que son silence vaut absence de probleme. Et un troisieme texte, une
annexe, consigne l'arbitrage de ce meme couple avec une issue differente de celle qui est codee ;
le registre prime sur l'annexe, mais aucune question ouverte ne demande de la realigner, si bien
que ce desaccord n'a pas de porteur. La lecon vaut au-dela : quand deux textes se contredisent,
ecrire la moitie juste donne l'assurance que l'affaire est close. Deuxieme fait mesure, sur le
second tour : un message d'infrastructure lu dans un journal de CI peut etre un ARTEFACT de test —
ici « Could not find a working container runtime strategy » etait la face rouge fabriquee du
temoin a deux faces, qui recopie la sortie de son sous-processus dans son propre echec, alors que
le demon repondait. Et pour capter ce qu'une route ecrit sans toucher l'objet global que la charte
du dossier d'integration refuse, on detourne le PORT que le type expose deja —
`frontiereDeProduction()` est exportee, son puits se remplace par etalement — plutot que
d'instrumenter l'hote.
