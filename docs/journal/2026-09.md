...
**Fait.** `src/server/integrations/axionia/api-entrante.ts` pose la frontiere unique des API qu'axionia appelle. Tout appel suit le meme chemin : configuration, adresse du client, jeton porteur, methode et route, debit, SIREN, puis lecture. Chaque refus est le meme — un 404, corps vide, aucun en-tete, identique octet pour octet quelle qu'en soit la cause. Un 401 confirmerait que la route existe, un 405 et l'OPTIONS automatique aussi : les deux routes exportent donc les sept methodes, et un attrape-tout rend le meme refus a un chemin inexistant sous le prefixe. Le jeton dedie est compare en temps constant, empreintes des deux cotes puis comparaison sans court-circuit sur la longueur. La liste d'adresses est fermee par defaut : absente, vide ou illisible, personne n'entre ; elle est normalisee par le lecteur de SEC-10, jamais par une copie. Echec ferme partout : le debit par defaut REFUSE, et la lecture d'attribution non branchee refuse aussi — jamais « libre » par defaut, ce serait echouer ouvert contre l'apporteur. Ce qui n'est pas dans la reponse ne peut pas fuir : le corps est reconstruit champ par champ depuis un schema strict, et un SIREN inconnu comme un SIREN libre rendent la meme constante au meme instant, plancher de duree et horloge injectee — la forme ET le temps sont indiscernables. Le journal est une ligne par appel, reconstruite depuis des champs fermes ; ni le jeton, ni l'adresse, ni un message d'erreur. 22 cas confrontes aux routes lues sur le disque, dont 98 appels pour deux routes fois sept methodes fois sept refus.

**Reste.** Le compteur de soixante par minute ne peut pas entrer au registre de SEC-10 tant que le texte de son exigence ne le chiffre pas sous la forme que la garde de famille lit : le debit refuse donc par defaut, et la question est ouverte au gardien du spec. La correspondance des etats vers le vocabulaire public appartient a une autre tache, d'ou le refus de la lecture d'attribution. La seconde moitie de l'exigence d'integration — cote axionia, delai de deux secondes, cache de cinq minutes, echec ouvert avec un devis cree non verifie et une alerte, verification rejouee par un job — appartient a la tache jumelle et n'est pas fermee ici, alors que la vue de tracabilite porte desormais le temoin de cette PR en face de cette exigence. Le controle de methode n'est exerce par aucun temoin : la forme exacte que l'en-tete dit vouloir interdire passe au vert, apres l'authentification. Le debit de la frontiere est un port prive hors du registre des compteurs. La comparaison en temps constant est la seule du depot et n'est mesuree par rien. Un oracle temporel leger subsiste sur l'appartenance a la liste, mesure dans le bruit. L'empreinte d'adresse est posee ici faute de module d'empreintes partage. Le 429 sans instant de reprise et la levee du limiteur n'ont aucun temoin. Enfin la gate declare une preuve rouge nulle alors que sa fixture rouge est mesurable, la ou ses deux soeurs declarent la leur.

**Appris.** Deux exigences se contredisent sur ce que la frontiere doit rendre, et la preseance tranche en faveur de celle de l'integration : le code livre est juste, mais la PR applique une clause et ecarte l'autre en silence — un lecteur du seul code ne peut pas savoir qu'une clause a ete ecartee. Pire, le fichier ecrit noir sur blanc l'une des deux divergences et tait l'autre : il apprend donc au lecteur que son silence vaut absence de probleme. Et un troisieme texte, une annexe, consigne l'arbitrage de ce meme couple avec une issue differente de celle qui est codee ; le registre prime sur l'annexe, mais aucune question ouverte ne demande de la realigner, si bien que ce desaccord n'a pas de porteur. La lecon vaut au-dela : quand deux textes se contredisent, ecrire la moitie juste donne l'assurance que l'affaire est close.
