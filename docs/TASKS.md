...
**286 taches · 216.35 j estimes.**

| Phase | Taches | Jours | Terminees |
| --- | ---: | ---: | ---: |
| -1 — Gouvernance (prealable bloquant) | 39 | 23.75 | 39 |
| 0 — Socle technique | 112 | 85.85 | 35 |
...
### GOV-035 — docs/PLAN-STATE.md est la cinquieme vue de REQ-GOV-032, et la seule sans verificateur ✅ **fusionnee**
...
### GOV-031 — eslint.config.mjs porte « CE FICHIER N'A JAMAIS ETE EXECUTE », et c'etait vrai ✅ **fusionnee**
...
**Acceptation.** SOCLE DE MESURE DE TOUTE LA PHASE 0 : 35 taches sur 68 en dependent, une sur deux. (1) `vitest.config.ts` et `tests/setup.ts` existent, `pnpm test` decouvre les specs suivis, et le seuil de couverture de `src/domain/**` est DECLARE a 100 % lignes et 100 % branches (REQ-QA-002) — declare et applique a ce qui existe : `src/domain/` ne porte que trois fichiers au jour de l'ouverture, et le vert imprime le compte des fichiers reellement couverts, jamais la longueur d'une liste tapee. (2) L'etape de Gate A qui lance `pnpm test` ne porte pas `continue-on-error`, ni en litteral ni sous une forme evaluee ; la garde qui le juge est celle deja livree par GOV-031, cette tache n'en ecrit pas une seconde (RM-01). (3) ESLINT REND EN ERREUR, PAS EN AVERTISSEMENT (decision du 2026-09-16) : `no-console` vaut `error` et non `warn`, `noInlineConfig: true` interdit qu'un commentaire de fichier eteigne une regle, et les 13 avertissements mesures sur la PR 44 valent ZERO a la livraison — corriges, ou couverts par une derogation NOMMEE et motivee dans `eslint.config.mjs`, jamais par une regle eteinte en bloc. Une CI qu'on verdit en eteignant la regle mesure la regle eteinte. (4) REQ-QA-001 : le lint refuse toute I/O sous `src/domain/**` (client de base, cache, appel reseau, horloge systeme). Les FORMES VOISINES de ces interdits — import de module sans prefixe, import dynamique, horloge atteinte par un autre chemin — ne sont PAS fermees ici : elles appartiennent a la tache de dette qui les porte, et l'acceptance de la livraison DIT explicitement laquelle des deux les ferme, pour qu'aucune des deux ne s'en remette a l'autre. (5) TEMOIN A DEUX FACES sur le desarmement de la gate, dans `tests/unit/ci/aucune-gate-en-continue-on-error.spec.ts` : une copie de travail de `.github/workflows/ci.yml` dont une seule etape de `gate-a` porte la tolerance d'echec fait sortir la garde en code NON NUL et NOMME l'etape fautive ; le `ci.yml` du depot la fait sortir en code ZERO. (6) TEMOIN A DEUX FACES sur le lint : un fichier de bac d'essai qui ecrit sur la console sous `src/domain/` fait sortir `pnpm lint` en code non nul ; le depot reel le fait sortir en zero, et `pnpm format:check` aussi.
...
**Acceptation.** TROISIEME TACHE LA PLUS BLOQUANTE DE LA PHASE (17 descendants). (1) `src/lib/env.ts` valide au demarrage, par un schema ferme, les SEPT secrets que REQ-SEC-028 enumere plus le sel de hachage d'adresse, chacun d'au moins 32 octets (64 caracteres hexadecimaux pour la cle de chiffrement). (2) AUCUNE valeur par defaut, AUCUN repli silencieux : une variable absente, trop courte, ou EGALE a une autre des huit fait echouer le demarrage avec un code de sortie non nul en moins de 60 s ; le message NOMME la variable et n'imprime ni sa valeur, ni un prefixe de sa valeur, ni sa longueur exacte. (3) L'EGALITE DE DEUX SECRETS EST UN REFUS A PART ENTIERE : huit noms qui pointent une meme valeur sont un seul secret, et c'est exactement ce que le mot « distincts » de REQ-SEC-028 interdit. Le controle se fait sur des empreintes, jamais en comparant les valeurs en clair dans un message. (4) `.env.example` porte les huit noms et aucune valeur utilisable ; il est DERIVE du schema, et une variable ajoutee au schema sans l'etre au fichier fait rougir (RM-01). (5) TEMOIN A DEUX FACES : un environnement de bac d'essai prive d'une des huit variables, puis un ou deux d'entre elles rendues egales, fait sortir le boot en code non nul et nomme la variable en cause ; l'environnement complet et distinct le fait sortir en zero, sans qu'aucun secret n'apparaisse dans la sortie. (6) La liste des huit vit a UN SEUL endroit du code ; `docs/gates.json` la cite sans la recopier.
...
**Acceptation.** (1) Toute reponse HTTP de Partners porte une politique de securite de contenu construite AUTOUR D'UN NONCE renouvele a chaque requete, `frame-ancestors 'none'`, `strict-dynamic`, un `connect-src` restreint aux origines que le code appelle reellement, plus HSTS avec preload, `X-Content-Type-Options`, `Referrer-Policy: strict-origin-when-cross-origin` et une `Permissions-Policy` restrictive (REQ-SEC-029). (2) AUCUNE tolerance en ligne : ni script ni style en ligne sans nonce ; si un composant en exige un, il porte le nonce, il ne fait pas lever la politique. (3) LE NONCE EST UNIQUE PAR REPONSE : deux requetes sur la meme route rendent deux nonces differents, et le nonce n'est jamais rendu dans une page mise en cache. (4) TEMOIN A DEUX FACES : une copie de travail de la couche d'en-tetes privee d'une seule directive, puis rendant deux fois le meme nonce, fait sortir la garde en code non nul et nomme la directive ou la repetition ; la couche du depot la fait sortir en zero, et le vert imprime le compte des routes reellement confrontees. (5) Le controle ne lit pas une liste tapee de routes : il DERIVE la liste des routes du disque, faute de quoi une route neuve echapperait a la mesure en silence.
...
**Acceptation.** (1) Les compteurs de limitation vivent sous les prefixes `magic:`, `depot:`, `verif:`, `webhook:`, `auth:`, et CHACUN declare sa conduite en cas de panne du cache : le depot est limite a 20 / 10 min par empreinte d'adresse avec `surPanne: laisser-passer`, le compteur par identite declare `surPanne: refuser` (REQ-SEC-016). (2) GARDE DE FAMILLE : un compteur ajoute sous l'un des cinq prefixes SANS declarer sa conduite sur panne est refuse — un predicat ouvert echoue ouvert, et c'est precisement ce qu'on ferme ici. (3) Le honeypot repond en succes silencieux et journalise l'identifiant d'apporteur SANS la valeur saisie ; l'accuse de depot n'est emis que si la ligne existe reellement (REQ-SEC-035). Aucun message, aucun code de reponse, aucun delai ne distingue le chemin honeypot du chemin nominal. (4) TEMOIN A DEUX FACES : une copie de travail ou un compteur neuf omet sa conduite sur panne, et une ou le cache est indisponible, font sortir la garde en code non nul et nomment le prefixe ; le code du depot la fait sortir en zero, et le vert imprime le compte des compteurs reellement confrontes. (5) Le detail des representations d'evitement ne s'ecrit pas ici : il vit hors depot, et cette acceptance ne le nomme pas.
...
**Acceptation.** (1) Tous les journaux applicatifs sont des lignes JSON structurees (pino) portant `requestId`, empreinte d'identifiant d'apporteur, `eventId` et `jobName` quand ils existent (REQ-QA-024). (2) LA REDACTION EST DANS LE JOURNALISEUR, PAS DANS LES APPELANTS : nom, prenom, telephone et adresse de courriel du contact rencontre, coordonnee bancaire et jeton de lien de depot sont remplaces au niveau du serialiseur ; aucun site d'appel n'a a y penser, parce qu'un site d'appel qui oublie est la regle et non l'exception. (3) Sentry recoit les erreurs avec le meme filtre, et le canal de notification refuse de partir hors production sans `NOTIFY_SINK=true` (impose par le hook d'environnement du depot). (4) TEMOIN A DEUX FACES : un journal de bac d'essai auquel on passe un objet portant chacun des champs proteges fait sortir la garde en code non nul tant que la redaction n'est pas posee, et NOMME le champ qui a franchi ; apres correctif, le meme objet fait sortir la garde en zero et la sortie ne contient aucune des valeurs passees. (5) Le vert imprime le compte des champs reellement confrontes ; « aucun champ en clair » sans le compte se lirait comme une absence prouvee.
...
**Acceptation.** DEUXIEME TACHE LA PLUS BLOQUANTE DE LA PHASE (19 descendants) ET PREMIERE ECRITURE DU SCHEMA — elle porte le label `schema`, l'approbation bloquante prevue par la charte, et elle est SEULE de sa famille dans son lot (une seule tache qui ecrit `prisma/schema.prisma` par lot, jamais deux). (1) Le socle du schema Partners existe, avec sa premiere migration. Toute colonne monetaire est un entier en centimes, HT, signee, suffixee `Cents` : aucune colonne a virgule flottante ni decimale, aucune valeur en euros (REQ-DM-001). Toute colonne dont le nom porte statut, type, motif, resultat, etat, origine, kind ou palier est un enum, jamais une chaine libre (REQ-DM-038). (2) LE JOURNAL `Evenement` EST APPEND-ONLY ET CHAINE : `prevHash`, `selfHash` calcule sur la charge canonique, et un declencheur de base REFUSE toute mise a jour et toute suppression (REQ-DM-024, REQ-JUR-026, REQ-SEC-027). Le refus est celui de la BASE, pas celui du code : un client qui contourne la couche applicative se heurte quand meme au declencheur. (3) LA CHARGE D'UN EVENEMENT EST UN SCHEMA FERME PAR TYPE — identifiants, empreintes, enums, montants, horodatages — et JAMAIS un nom, une adresse de courriel, un telephone, une coordonnee bancaire ou une adresse postale (REQ-DM-041). (4) APRES UN EFFACEMENT DE TIERS, LA CHAINE RESTE VERIFIABLE : c'est la consequence directe de (3), et elle se prouve, elle ne se postule pas. (5) TEMOIN A DEUX FACES, en base reelle : une tentative de mise a jour et une tentative de suppression d'une ligne du journal echouent toutes deux et nomment le declencheur ; une insertion nominale reussit, et `verifierChaine()` sort en zero sur la base ainsi remplie. (6) TEMOIN A DEUX FACES sur la charge : une charge de bac d'essai portant un champ interdit fait sortir la garde `journal:sans-pii` en code non nul et nomme le champ ; les charges du depot la font sortir en zero, avec le compte des types reellement confrontes.
...
**Acceptation.** ELLE ABSORBE LA DETTE MESUREE LE 2026-09-16 SUR LA GARDE DE SCHEMA ET D'ENUMS (decision du gardien-spec : une tache neuve aurait duplique le porteur — `partners:schema:enums` et `partners:schema:cents` sont deja attribuees ici). Estimation relevee de 0,5 a 1,5 j pour cette raison. ETAT MESURE : `scripts/gates/schema-enums.ts` est la seule implementation depuis partners/ADR-0011, et elle lit Prisma et le code par des motifs qui MANQUENT des declarations valides — un modele lu jusqu'a la premiere accolade fermante meme rencontree dans un commentaire, et tous les champs suivants echappent a REQ-DM-038 ; une enumeration d'etats ecrite sur plusieurs lignes et des membres d'enum non entoures de guillemets ne sont pas vus, parce que la recherche se fait ligne a ligne ; invoquee sans extension, la garde sort en zero sans rien ecrire. Le schema reel arrive avec DM-01 : la garde doit etre juste AVANT d'avoir quelque chose a juger. A LIVRER. (1) La lecture de `prisma/schema.prisma` ne se fait plus ligne a ligne : commentaires et chaines sont retires AVANT le decoupage des modeles, et un modele se ferme sur sa vraie accolade. (2) Une garde invoquee sur un perimetre VIDE ne sort pas en zero en silence : elle imprime le perimetre et son compte, et « 0 fichier balaye » ne se lit pas comme « aucun defaut ». (3) Les cinq controles que le registre annonce s'arment : entiers en centimes (REQ-DM-001), vocabulaires en enums (REQ-DM-038, REQ-JUR-027), index unique PARTIEL sur l'attribution occupante ecrit en SQL brut et verifie en lisant le catalogue d'index APRES le deploiement des migrations (REQ-DM-003), migrations additives sans suppression de colonne ni changement destructif d'enum sans decision ecrite (REQ-DM-037). (4) TEMOIN A DEUX FACES pour CHACUN des cinq : une copie de travail du schema portant le defaut — une colonne monetaire a virgule flottante, un vocabulaire en chaine libre, un champ place apres une accolade commentee, un index unique total la ou l'exigence demande un partiel, une migration qui supprime une colonne — fait sortir la garde en code non nul et nomme le modele et le champ ; le schema du depot la fait sortir en zero. (5) Le vert imprime le compte des modeles et des champs REELLEMENT confrontes.
...
**Acceptation.** TACHE QUI ECRIT LE SCHEMA — label `schema`, seule de sa famille dans son lot. (1) `src/lib/env.ts` valide strictement toutes les variables au demarrage ; une variable manquante EMPECHE le demarrage, code de sortie non nul (REQ-QA-030). `docs/env.md` est GENERE depuis le schema et une garde rougit s'il est perime (RM-01). (2) `/api/livez` repond 200 des que le processus repond. `/api/readyz` repond 503 si la base, le cache, la validation d'environnement OU des migrations en attente sont en defaut, et 200 sinon ; les TROIS cas de panne ont chacun leur test (REQ-QA-020). (3) Le `HEALTHCHECK` de l'image et la sonde de la plateforme pointent sur `readyz`, jamais sur `livez` : une sonde qui interroge la vie du processus declare saine une instance qui ne sait pas servir. (4) L'entrypoint execute `prisma migrate deploy` en mode bloquant : un echec sort en code non nul en moins de 60 s et l'ancien conteneur reste servi ; l'echappatoire `SKIP_MIGRATE=1` n'est utilisee que par le runbook de retour arriere, et une garde le prouve (REQ-QA-019). (5) Hors production, `NOTIFY_SINK=true` est obligatoire, refus au demarrage sinon (REQ-CPL-021). (6) TEMOIN A DEUX FACES : conteneur demarre avec la base coupee, puis avec le cache coupe, puis avec une migration en attente — `readyz` rend 503 dans les trois cas et NOMME le sous-systeme en defaut ; tout remis en place, il rend 200 et `livez` rend 200 dans les quatre situations.
...
**Acceptation.** (1) `.semgrep.yml` porte les regles publiques retenues ET les regles maison, chaque regle maison ayant SON test — une regle sans test est une regle qu'on croit active. (2) La regle maison qui compte le plus ici interdit l'usage direct du client de base hors de la couche d'acces cloisonnee pour les modeles cloisonnes (REQ-QA-011) : c'est elle qui rend SEC-05 verifiable, et elle est ecrite ici meme si la couche n'existe pas encore — une regle qui attend son code est une garde, une regle ecrite apres coup est un constat. (3) L'etape de Gate A qui lance semgrep ne porte pas de tolerance d'echec (REQ-QA-013), et la version de l'outil est epinglee. (4) TEMOIN A DEUX FACES : un fichier de bac d'essai qui enfreint chaque regle maison, un par un, fait sortir semgrep en code non nul et nomme la regle ; le depot reel le fait sortir en zero, et le vert imprime le compte des regles reellement exercees, jamais la longueur de la liste declaree. (5) Une regle desactivee par un commentaire en ligne est refusee : le perimetre ne se retrecit pas fichier par fichier.
...
**Acceptation.** (1) `stryker.config.json` existe, porte sur `src/domain/**` seul, et tourne dans le travail de nuit, pas dans Gate A — le cout ne tient pas dans une PR. (2) LE SEUIL EST DECLARE ET IL EST TENU : le score de mutation minimal est ecrit dans la configuration, et un score inferieur fait sortir le travail de nuit en code non nul. Un seuil declare qu'aucune sortie ne fait respecter est une intention. (3) LES SURVIVANTS SONT NOMMES, PAS COMPTES : la sortie enumere chaque mutant survivant avec son fichier et sa ligne. Un compte seul ne dit pas quoi corriger, et c'est le defaut deja rencontre trois fois dans ce depot sur des comptes tapes. (4) TEMOIN A DEUX FACES : retirer une assertion d'un test de `src/domain/**` fait passer le score sous le seuil et fait sortir le travail de nuit en code non nul en nommant le mutant devenu survivant ; l'etat du depot le fait sortir en zero. (5) REQ-QA-002 : la couverture a 100 % lignes et branches reste la condition d'entree ; le score de mutation est ce qui empeche cette couverture d'etre atteinte par des tests qui n'affirment rien.
...
**Acceptation.** TACHE SENSIBLE — donnees personnelles. (1) Nom, adresse de courriel et telephone du tiers rencontre et de l'apporteur, ainsi que la coordonnee bancaire, sont chiffres en AES-256-GCM avec une donnee additionnelle authentifiee EGALE a l'identifiant de la ligne : un bloc chiffre deplace d'une ligne a une autre ne se dechiffre pas (REQ-SEC-024). (2) Les recherches et les comparaisons passent par des empreintes salees (`emailHash`, `phoneHash`, `ibanHash`, `siretHash`) ; aucune de ces empreintes n'est reversible, et AUCUNE ADRESSE RESEAU N'EST STOCKEE EN CLAIR. (3) LE SEL ET LA CLE VIENNENT DE SEC-01 : ils ne sont ni ecrits dans le code, ni derives d'une autre variable, ni partages entre deux usages. (4) LE CHIFFREMENT EST DANS LA COUCHE DE PERSISTANCE : un site d'appel ne peut pas ecrire un champ protege en clair, parce que le chemin d'ecriture ne le lui permet pas. (5) TEMOIN A DEUX FACES : une ligne chiffree dont on transpose le bloc vers une autre ligne echoue au dechiffrement et NOMME l'echec d'authentification ; la meme ligne a sa place se dechiffre et sort en zero. (6) TEMOIN A DEUX FACES sur la persistance : un chemin d'ecriture de bac d'essai qui tente d'ecrire un champ protege en clair est refuse et nomme le champ ; le chemin du depot sort en zero. (7) Le vert imprime le compte des champs et des chemins d'ecriture reellement confrontes.
...
**Acceptation.** (1) Une PORTE D bloquante applique les migrations sur une base VIERGE et sur un vidage de la version precedente, exige que la difference de schema calculee apres coup soit VIDE, et demarre l'application contre la base ainsi migree (REQ-QA-021). (2) EXPAND PUIS CONTRACT : aucune PR ne contient une suppression de colonne, un renommage, ni une contrainte de non-nullite sans valeur par defaut sur une colonne encore lue par le code deploye. La garde lit les migrations de la PR, pas le schema final. (3) LE VIDAGE DE LA VERSION PRECEDENTE EST SEME, pas vide : une migration additive passe trivialement sur une base vide, et c'est le faux vert de cette famille. (4) TEMOIN A DEUX FACES : une migration de bac d'essai qui supprime une colonne encore lue fait sortir la porte D en code non nul et nomme la colonne et le fichier qui la lit ; les migrations du depot la font sortir en zero, et le vert imprime le compte des migrations reellement confrontees. (5) L'etape de Gate A qui lance la porte D ne porte pas de tolerance d'echec.

**Tests.** `tests/integration/migrations-additives.spec.ts`
...
**Acceptation.** TACHE SENSIBLE — argent et attribution. UN REFUS DE LA LENTILLE SECURITE Y VAUT VETO. (1) Le calcul de commission est une FONCTION PURE de `src/domain/**` : aucune entree-sortie, aucune horloge, aucune lecture de la base. Sa signature prend une version de grille, une activite de facturation, un nombre de journees eventuellement nul et un montant HT en centimes, et rend soit un montant en centimes, soit un BLOCAGE motive (REQ-DM-015). (2) LES CINQ VALEURS D'ACTIVITE SONT COUVERTES, et un cas sans bareme produit le blocage « bareme introuvable » — jamais zero. Zero est un montant ; l'absence de bareme n'en est pas un. (3) LE FORFAIT SE COMPTE UNE FOIS PAR COMMANDE PORTANT LE PALIER, JAMAIS MULTIPLIE PAR LES JOURNEES (REQ-ARG-006, corrige le 2026-09-03 : la formule anterieure doublait la commission). Un test nomme cette correction et la tient. (4) LE PRORATA EST ENTIER ET S'ECRIT UNE SEULE FOIS DANS LE DEPOT : part acquise au ieme encaissement = partie entiere de (commission totale en centimes x cumul encaisse TTC en centimes / montant TTC net de la facture en centimes) moins la somme des parts deja acquises ; numerateur et denominateur sont de MEME NATURE (REQ-DM-017, REQ-ARG-004). (5) INVARIANTS PROUVES, PAS POSTULES : a facture soldee la somme des parts vaut exactement la commission totale ; aucune part n'est negative ; aucune part ne depasse la commission totale ; toute permutation de l'ordre des encaissements d'un meme dossier rend le meme etat final. (6) PLAFOND : une commission superieure au pourcentage plafond du montant HT de la ligne bloque la ligne avec le motif d'enum prevu, et ce blocage ne se leve que par une decision journalisee (REQ-ARG-007, REQ-ARG-017). (7) TEMOIN A DEUX FACES : un jeu de bac d'essai ou l'arrondi est fait en flottant, puis un ou le denominateur est un montant HT quand le numerateur est TTC, font sortir les tests d'invariant en code non nul et NOMMENT l'invariant rompu et l'ecart en centimes ; l'implementation du depot les fait sortir en zero sur le meme jeu.
...
**Acceptation.** DEPOT AXIONIA. TACHE SENSIBLE — attribution. (1) Les evenements `client.cree` et `client.mis_a_jour` sont produits par une FONCTION D'EMISSION UNIQUE, et TOUS les ecrivains de Client y passent (REQ-INT-007). (2) UN TEST-CLIQUET NOMINATIF enumere les ecrivains trouves dans le code d'axionia et ROUGIT si l'un d'eux n'emet pas. Il enumere par IDENTITE, pas par compte : un ecrivain ajoute dans un fichier deja declare doit faire rougir, et c'est la faute exacte mesuree sur le cliquet des refus le 2026-09-14. (3) Le SIREN est normalise a 9 chiffres avec son controle de cle, et DERIVE du SIRET quand il est vide (REQ-INT-015) ; un SIREN invalide n'est jamais transmis tel quel. (4) La resolution d'un encaissement cherche l'attribution occupante du SIREN beneficiaire transmis, sinon un rattachement manuel actif ; sans SIREN beneficiaire ni facture, la ligne est bloquee avec son motif d'enum et une anomalie est ouverte — l'evenement n'est JAMAIS ignore (REQ-DM-021). (5) TEMOIN A DEUX FACES : un ecrivain de Client ajoute dans un fichier deja declare, qui n'appelle pas la fonction d'emission, fait sortir le cliquet en code non nul et NOMME le fichier et la ligne ; le code du depot le fait sortir en zero avec le compte des ecrivains reellement confrontes.
...
**Acceptation.** DEPOT AXIONIA. TACHE SENSIBLE — argent. UN REFUS DE LA LENTILLE SECURITE Y VAUT VETO. (1) Un evenement `devis.signe` UNIQUE est produit quel que soit le chemin par lequel un devis passe a l'etat accepte : tous les ecrivains de cet etat passent par la fonction d'emission unique (REQ-INT-007). (2) La charge porte, PAR LIGNE, l'identifiant de palier de commission ou nul, le montant HT en centimes, l'activite et la designation, CALCULES DANS AXIONIA par une fonction pure derivee de la source de la grille (REQ-INT-006). PARTNERS NE CONTIENT AUCUNE COPIE DE LA GRILLE, et c'est ce que cette tache rend vrai du cote emetteur. (3) Un identifiant de palier qui ne resout pas dans la grille publiee est transmis comme NUL avec le motif, jamais devine : une commission calculee sur un palier invente est une perte d'argent silencieuse. (4) TEMOIN A DEUX FACES : un chemin d'acceptation de devis ajoute sans passer par l'emission fait sortir le cliquet en code non nul et nomme le fichier ; le code du depot le fait sortir en zero. (5) TEMOIN A DEUX FACES sur la charge : une ligne dont le montant HT est exprime en euros et non en centimes est refusee au schema de sortie avec le nom du champ ; les lignes du depot passent.
...
**Acceptation.** TACHE SENSIBLE — authentification. (1) Les routes appelees par axionia repondent 404 — jamais 401 — sans jeton porteur dedie d'au moins 32 octets, compare en temps constant ; elles sont restreintes par une liste d'adresses autorisees, limitees a 60 appels par minute, et chaque appel est journalise (REQ-SEC-012). 404 PLUTOT QUE 401 : un 401 confirme que la route existe. (2) La route d'interrogation d'attribution rend une reponse MINIMALE — un statut parmi libre, attribuee ou cliente, une echeance eventuelle — et NE REND NI NOM NI ADRESSE DE COURRIEL D'APPORTEUR (REQ-INT-014). Ce qui n'est pas dans la reponse ne peut pas fuir. (3) La reponse est IDENTIQUE en forme et en temps pour un SIREN inconnu et pour un SIREN libre : une difference mesurable est un oracle d'enumeration. (4) TEMOIN A DEUX FACES : un appel sans jeton, puis avec un jeton faux, puis depuis une adresse hors liste, rendent 404 et n'apparaissent dans aucune reponse sous une autre forme ; l'appel autorise rend 200 avec exactement les champs que le contrat declare, et un test de forme refuse tout champ en trop. (5) Le vert imprime le compte des routes reellement confrontees.
...
**Acceptation.** TACHE CHARNIERE DU CLOISONNEMENT — 8 exigences. (1) TOUTE lecture et TOUTE ecriture de l'espace apporteur passent par une couche d'acces qui INJECTE l'identifiant d'apporteur de la session dans la clause de filtrage ; aucun import direct du client de base sous `src/app/(espace)/**` ni sous `src/server/espace/**` (REQ-SEC-008, REQ-QA-011). La regle semgrep qui le tient est celle livree par QA-T07 : cette tache la CONSOMME, elle n'en ecrit pas une seconde. (2) UN TEST DE CLOISONNEMENT EN BOITE NOIRE, BLOQUANT, appelle CHAQUE route et CHAQUE action de serveur de l'espace avec l'identifiant d'un AUTRE apporteur, documents PDF compris, et exige un 404 STRICTEMENT IDENTIQUE a celui d'un identifiant inexistant (REQ-SEC-009, REQ-QA-010, REQ-ARG-029, REQ-UX-006). Identique veut dire : meme corps, meme en-tetes significatifs, meme duree observable. (3) UNE GARDE STATIQUE FAIT ROUGIR LA CI SI UNE ROUTE OU UNE ACTION NEUVE N'EST PAS DANS LA LISTE DES CAS : la liste se DERIVE du disque et se confronte a la liste declaree ; une route neuve non listee est un trou, pas un oubli. (4) Le message de collision et la reponse « deja suivie » ne contiennent NI identifiant, NI nom, NI date de depot, NI stade de l'autre apporteur — seule une date de fin est admise (REQ-QA-012), et une comparaison de forme de la reponse le prouve. (5) TEMOIN A DEUX FACES : une route ajoutee sans son cas de cloisonnement fait sortir la garde statique en code non nul et nomme la route ; le depot complet la fait sortir en zero avec le compte des routes et des actions reellement confrontees. (6) TEMOIN A DEUX FACES sur l'indistinction : la reponse a un identifiant d'un autre apporteur et celle a un identifiant inexistant sont comparees octet a octet.
...
**Acceptation.** DEPEND D'UN TIERS : l'API publique de recherche d'entreprises. Aucune cle, mais des quotas, et un REPLI MANUEL OBLIGATOIRE. (1) L'autocompletion passe par un mandataire SERVEUR : anti-rebond de 300 ms, requete minimale, cache de 24 h, limite globale de 5 requetes par seconde, et respect de l'en-tete de temporisation renvoye par le tiers (REQ-INT-020). (2) SUR PANNE DU TIERS — refus pour exces, delai depasse, erreur serveur — LE DEPOT BASCULE EN SAISIE MANUELLE de SIREN ou SIRET controlee, il ne se bloque pas : un parcours d'apporteur qui depend de la disponibilite d'un tiers est un parcours qui s'arrete quand le tiers s'arrete. Le disjoncteur ouvre, et l'etat ouvert est VISIBLE en console. (3) LE MANDATAIRE EST LIMITE PAR IDENTITE (120 par jour) ET PAR EMPREINTE D'ADRESSE, journalise, et SA REPONSE NE CONTIENT JAMAIS LA LISTE DES DIRIGEANTS (REQ-SEC-013) ; le serveur peut les conserver en base pour un signal interne, il ne les renvoie pas au navigateur. AUCUNE ANNEE DE NAISSANCE NE TRAVERSE, et une garde dediee le tient. (4) Le client est valide par schema contre des FIXTURES ENREGISTREES ET DATEES, au moins 20 cas, et un test de contrat NOCTURNE contre l'API reelle signale toute derive de forme (REQ-QA-028). Les fixtures viennent du producteur reel, jamais d'une main (regle maison n°3). (5) Les champs persistes sont exactement ceux que REQ-INT-021 enumere, et un test de forme refuse tout champ en trop. (6) TEMOIN A DEUX FACES : une reponse de bac d'essai portant une liste de dirigeants, puis une portant une annee de naissance, font sortir la garde en code non nul et nomment le champ ; les 20 fixtures enregistrees la font sortir en zero. (7) TEMOIN A DEUX FACES sur le repli : le tiers simule en refus pour exces fait basculer le parcours en saisie manuelle et le test le CONSTATE sur le rendu, pas sur un journal ; le tiers disponible rend l'autocompletion.
...
**Acceptation.** (1) L'adaptateur de la console federee expose une route unique, protegee par un SECRET PROPRE a Partners : sans secret configure la route rend 503, avec un secret faux elle rend 401, et LE LIMITEUR S'APPLIQUE AVANT LA SERRURE — sinon la serrure devient l'oracle de debit (REQ-INT-026). (2) Le contrat du socle est porte A L'IDENTIQUE, avec le sceau des profils verifie, et le manifeste est VERSIONNE : une divergence entre le manifeste publie et le contrat du socle est un refus nomme, pas une adaptation silencieuse. (3) LE HARNAIS DE 9 CONTROLES EST EXECUTABLE D'UNE COMMANDE et imprime, controle par controle, ce qu'il a confronte. Un harnais qui rend « 9/9 » sans dire quoi ne prouve rien — c'est la famille des trois voyants verts sans envoi. (4) TEMOIN A DEUX FACES : le harnais lance contre un adaptateur prive de son secret, puis avec un secret faux, puis avec le limiteur place apres la serrure, sort en code non nul et NOMME le controle en defaut ; lance contre l'adaptateur du depot, il sort en zero avec les 9 controles nommes et leur resultat.
...
**Acceptation.** DEPEND D'UN TIERS : un bot DEDIE a Partners (jamais celui d'axionia), son jeton et l'identifiant de la conversation de console. TACHE SENSIBLE — espace et donnees personnelles. (1) Les alertes de console passent par ce bot dedie, avec DEDOUBLONNAGE et PLAFOND HORAIRE PAR CATEGORIE : une alerte qui se repete 400 fois dans l'heure fait desarmer le canal par celui qui le lit, et c'est ainsi qu'une alerte utile se perd (REQ-INT-024). (2) AUCUN MESSAGE NE CONTIENT DE COORDONNEE DE TIERS NI DE LIEN DE CONSOLE : ni nom, ni adresse de courriel, ni telephone, ni URL d'administration. Un canal de messagerie n'est pas un canal de confiance. (3) Le message porte de quoi RETROUVER l'objet — identifiant technique, categorie, compte — et rien de plus. (4) TEMOIN A DEUX FACES : un message de bac d'essai construit a partir d'un objet portant nom, courriel, telephone et lien de console fait sortir la garde en code non nul et NOMME le champ qui a franchi ; les messages du depot la font sortir en zero, avec le compte des gabarits de message reellement confrontes. (5) TEMOIN A DEUX FACES sur le plafond : 200 alertes de la meme categorie dans l'heure produisent au plus le nombre declare d'envois, et le test compte les envois.
...
**Acceptation.** DECOUPEE LE 2026-09-16. La premiere redaction declarait une EXEMPTION au controle « aucune tache ne depend d'une tache de phase strictement superieure », au motif que les libelles d'etats d'attribution viennent de DM-07, qui est en phase 1. Cette exemption n'existe dans aucun registre, et elle n'y sera pas posee : une exemption est un trou PERMANENT, deux taches sont un fait. La premiere redaction le recommandait elle-meme. Cette tache-ci ne porte donc QUE ce qui est livrable des maintenant, et sa dependance a GOV-006 est sa seule dependance. A LIVRER. (1) La micro-copie de l'espace a une source unique : chaque libelle vit dans `messages/fr.json` ou sous `src/content/micro-copy/`, et nulle part ailleurs — aucun libelle en dur dans un composant. (2) Chaque issue possible d'un depot, telle que l'enum la declare, affiche un TITRE, un POURQUOI et un QUOI FAIRE, et dit explicitement si le depot est horodate au nom de l'apporteur (REQ-UX-002). L'enum est aligne un pour un sur les categories du contrat : une issue ajoutee au contrat sans son texte fait rougir. (3) CHAQUE ECRAN DE L'ESPACE ET DE LA CONSOLE A UN ETAT VIDE DECLARE — titre, phrase, action principale — et celui de la liste des entreprises guide vers le premier depot (REQ-UX-019). Un ecran sans etat vide declare est un ecran qui, le premier jour, ne dit rien a personne. (4) AUCUN TEXTE VU PAR UN APPORTEUR ne contient les mots que REQ-UX-003 refuse ; la liste est IMPORTEE de la source unique du lexique, jamais recopiee dans la garde. (5) TEMOIN A DEUX FACES : une valeur ajoutee a l'enum des issues sans son texte, un ecran sans etat vide declare, et un libelle en dur dans un composant font sortir la garde en code non nul, un cas par un, en NOMMANT la valeur, l'ecran ou le fichier ; la micro-copie du depot la fait sortir en zero, avec le compte des issues, des ecrans et des fichiers reellement confrontes. CE QUI N'EST PAS ICI : les libelles d'etats d'attribution, qui dependent de DM-07 — ils vivent en UX-P0-01b, phase 1.
...
**Acceptation.** HARNAIS DE NAVIGATEUR. La phase 0 ne produit que DEUX pages reelles : `src/app/(espace)/connexion/` (SEC-03) et la page de confidentialite (JUR-T04). LE HARNAIS S'EXERCE DONC SUR CES DEUX PAGES ET SUR LES 6 MAQUETTES `docs/maquettes/*.html` de UX-P0-02 — decision du 2026-09-16 : les trois gates d'accessibilite, de cibles et de reflux deviennent BLOQUANTES SUR LES ROUTES REELLES A PARTIR DE LA PHASE 1, et ne le sont pas avant ; ce report est ECRIT dans `docs/gates.json`, avec la tache qui l'annule. Une gate non bloquante dont personne ne sait qu'elle l'est est un faux vert, et c'est la septieme occurrence de cette famille dans ce depot. (1) `playwright.config.ts` existe et declare les projets mobiles exiges par REQ-QA-016 pour l'espace (iPhone 14 Pro et Pixel 7) et le projet de bureau pour la console ; UN SPEC DE PARCOURS D'ESPACE SANS PROJET MOBILE FAIT ROUGIR, et la garde DERIVE la liste des specs du disque. ⚠️ ARBITRAGE DU 2026-09-16, SANS LEQUEL CETTE TACHE SE CONTREDIT : REQ-QA-016 dit « projets mobiles UNIQUEMENT » et REQ-UX-017 exige l'audit d'accessibilite « profil iPhone ET desktop ». Prises a la lettre, les deux ne tiennent pas ensemble, et la preseance ne tranche pas (ni QA ni UX n'est dans DM > INT > SEC). Le « uniquement » de REQ-QA-016 porte sur les PARCOURS FONCTIONNELS de bout en bout ; il ne porte pas sur les passes d'accessibilite, qui sont un controle de RENDU et non un parcours. Les trois specs de `tests/a11y/` s'executent donc sur les deux profils, les specs de parcours sur les projets mobiles seuls. Le texte de REQ-QA-016 est amende en consequence dans le meme lot ; sans cet amendement la contradiction se rejouera a chaque relecture. (2) `tests/a11y/axe.spec.ts` : zero violation grave ou critique sur chacune des huit surfaces (REQ-UX-017, REQ-UX-018). (3) `tests/a11y/cibles.spec.ts` : toute cible interactive mesure au moins 48 x 48 pixels CSS dans l'espace ; le corps de texte est au moins a 18 px avec un contraste d'au moins 4,5:1. (4) `tests/a11y/reflow.spec.ts` : a 320 px de large et a 200 % de zoom, AUCUNE des huit surfaces ne fait apparaitre de defilement horizontal. (5) LE DOM SE MESURE AU PREMIER PLAN : une page mesuree dans un onglet d'arriere-plan rend des zeros, et des zeros passent tous les seuils. Le harnais le garantit et le dit dans sa sortie. (6) TEMOIN A DEUX FACES : une surface de bac d'essai portant un bouton de 24 px, un contraste insuffisant et un debordement a 320 px fait sortir les trois specs en code non nul et NOMME la surface et le selecteur fautif ; les huit surfaces du depot les font sortir en zero, et le vert imprime le compte des surfaces et des cibles reellement mesurees.

**Tests.** `tests/a11y/axe.spec.ts` · `tests/a11y/cibles.spec.ts` · `tests/a11y/reflow.spec.ts`
...
**Tests.** `tests/unit/qualite/budget-par-route.spec.ts`
...
**Acceptation.** ETAT MESURE le 2026-09-12. Le champ `attestation { pr, sha de 40 hex, fusionneeAt }` EXISTE, il est declare au schema, `lot:cloture` REFUSE le passage `fusionnee` sans ses trois valeurs, et `gov:tasks` en juge la forme (familles attestation_sha_non_conforme, attestation_date_non_conforme). Mais tout cela ne s'arme QUE si le depot de la tache n'est pas `partners` : la condition est dans `scripts/lot/cloture.ts` et gouverne tout le bloc. Mesure : 1 tache porte une attestation (INT-T01b, chez axionia) ; 33 taches `fusionnee` en `repo: partners`, ZERO avec un sha. Une tache locale passe donc `fusionnee` avec un `pr` nu, et rien ne conserve la trace du commit qui l'a fait atterrir. Consequence mesuree le 2026-09-09 sur la chaine de gardes d'alors : GOV-035 passee `fusionnee` A LA MAIN dans `docs/tasks.json`, sans aucun `resultat.json`, la laissait entierement verte — le `deny` de la configuration de session protege l'outil d'un agent, il ne protege pas le fichier. Et l'invariant qui aurait pu le voir est tenu par un fichier que le depot ne VOIT pas : `docs/lots/` est hors suivi, verifie par `git log --all --diff-filter=A -- docs/lots` qui ne rend rien sur aucune branche, et seul `rendre-resultat-lot.mjs`, hors depot, mesure l'ancestralite du commit. LA TACHE ETEND l'attestation aux taches locales — elle n'en invente pas une deuxieme forme (RM-01) — et une garde confronte les deux populations dans les DEUX SENS : un statut `fusionnee` sans attestation, une attestation sans statut. Contre-temoin obligatoire : les taches livrees hors depot, qui portent deja la leur, doivent RESTER vertes.

⚠️ AMENDEMENT DU 2026-09-16 — LE RATTRAPAGE DU PASSE, SANS LEQUEL CETTE TACHE NE PEUT PAS SE FERMER. Etendre l'attestation aux taches locales la rend exigible sur les 33 taches de phase -1 deja `fusionnee` chez `partners`, dont AUCUNE ne porte de sha : la garde rougirait le jour de sa livraison sur un passe qu'elle ne peut pas reecrire. Le sha manquant NE S'INVENTE PAS et NE SE DEVINE PAS : il se LIT dans l'historique de la branche par defaut, en cherchant le commit d'ecrasement qui NOMME la tache (`git log --format=%H --grep '<ID>' origin/main`), et le rattrapage ECHOUE BRUYAMMENT quand la recherche rend zero commit ou plus d'un — une tache dont le commit d'atterrissage est ambigu reste SANS attestation et le dit, plutot que de porter un sha plausible. Un enregistrement fabrique est pire que l'enregistrement absent qu'il remplace : c'est exactement ce que cette tache reproche a l'etat actuel. Contre-temoin exige : une tache dont le motif de recherche rend deux commits laisse l'attestation vide, la garde la NOMME, et le vert final imprime le compte des taches reellement rattrapees, jamais la longueur d'une liste tapee.
...
**Acceptation.** LE NEUVIEME TROU, ET LE SEUL QUE LE LOT L-1-05 A RENCONTRE EN FAISANT SON TRAVAIL — les huit autres etaient connus depuis le 2026-09-09. GOV-038, `en_cours`, portait `lot: null` : nee sur une branche orpheline avant le passage du composeur. La liste des taches d'une cloture se DERIVE du champ `lot` (`rendre-resultat-lot.mjs`, refus 1), parce que `docs/lots/` est hors suivi. Une tache `en_cours` sans lot n'est donc vue par AUCUNE cloture, et resterait `en_cours` pour toujours. MESURE PAR MUTATION le 2026-09-12, PUIS REFAITE INDEPENDAMMENT par la lentille exactitude sur un clone avec historique — les deux concordent : l'etat d'avant la PR reconstitue (GOV-038 remise a `lot: null`, statut `en_cours`), quatre vues regenerees, `pnpm gov:check` rend 16 bannieres vertes, exit 0 ; et dans la variante ou le statut est laisse `fusionnee`, il reste vert sans meme regenerer. Aucune famille de `gov-tasks.ts` ni de `gov-etat.ts` ne couvre le cas. LA REGLE VA AU SCHEMA, et c'est un changement de la premiere redaction, exige par la lentille simplicite : une clause JUMELLE y vit deja (`fusionnee` implique `owner` et `branch` requis), et GOV-050, versee dans la meme PR, argumente qu'une regle de cette forme va au schema — la poser dans une garde aurait rendu les deux taches contradictoires.

⚠️ AMENDEMENT DU 2026-09-16, DEUX CORRECTIONS MESUREES A LA CLOTURE DE LA PHASE -1.

PREMIERE CORRECTION — LA PORTEE. La clause s'applique a `en_cours` SEUL, et non « ou au-dela ». Prise a la lettre, la formule « ou au-dela » condamnerait SIX taches reellement livrees : INT-T01b, puis GOV-035, GOV-030, GOV-031, GOV-036 et GOV-037, fusionnees chacune par sa PR, toutes a `lot: null` parce que `docs/lots/` ne les a jamais vues. Ecrire la regle sans cette correction rendrait le registre rouge le jour meme de la cloture, et la seule facon de le verdir serait d'INVENTER des lots a posteriori — c'est-a-dire de fabriquer l'enregistrement que la regle pretend proteger. `lot: null` sur une tache livree hors lot est le choix mesure du plan ; ecrire un lot faux serait pire.

SECONDE CORRECTION — LE DISCRIMINANT N'EST PAS LA FORME DU NOM DE BRANCHE. Une premiere redaction exemptait « la tache qui porte une branche de la forme derogatoire `t/<slug>` ». MESURE DU 2026-09-16 : les six taches `fusionnee` a `lot: null` ne sont PAS toutes sur `t/` — CINQ le sont (GOV-030, GOV-031, GOV-035, GOV-036, GOV-037) et UNE porte `lot/int-t01b-contrat-evenements` (INT-T01b). La regle ecrite sur la forme du nom laisserait donc INT-T01b rouge, c'est-a-dire exactement le resultat que l'amendement veut eviter, et le texte qui affirme que les six sont en `t/` est FAUX : il est retire. LE DISCRIMINANT EST L'ABSENCE DE LOT ELLE-MEME, attestee autrement : une tache livree SEULE, hors de tout lot, est reconnue a ce qu'elle porte `pr` ET `branch` ET — des que GOV-042 est livree — son sha d'atterrissage. Un nom de branche est une convention d'humain, il se tape et se trompe ; une attestation est une preuve. CE QUI DOIT ROUGIR est la tache livree EN LOT dont le lot manque, pas la tache livree seule et attestee.
...
**Acceptation.** MESURE QUI OUVRE LA TACHE, jouee par la lentille mutation au tour de revue de la PR 36, a refaire avant d'ecrire une ligne : reecrire la rubrique « Prochain pas » de docs/PLAN-STATE.md en « la phase -1 est TERMINEE, 39/39 taches, plus aucune tache eligible », puis `pnpm plan-state:verifier` -> EXIT 0. Le mensonge passe.

POURQUOI. `plan-state:verifier` exempte EN ENTIER toute rubrique dont le generateur a lu la forge pour l'ecrire — la provenance est notee au moment de l'ecriture, il n'y a plus de liste — et « Prochain pas » lit l'etat des PR pour la tete de la file de fusion. Mais LA LIGNE QUI ENUMERE LES TACHES ELIGIBLES se derive de `docs/tasks.json` SEUL — statut `a_faire`, phase courante, `externe` nul, dependances toutes livrees — sans le moindre appel a la forge. Une rubrique qui mele une source vivante et une source du depot est donc declaree volatile EN ENTIER, et sa part derivable sort du controle avec le reste.

⚠️ AMENDEMENT DU 2026-09-16 — UNE RUBRIQUE SE DESIGNE PAR SON TITRE, JAMAIS PAR SON RANG. La premiere redaction disait « sa DEUXIEME ligne ». Ce rang est FAUX la moitie du temps : il designe le deuxieme paragraphe quand une PR est en tete de la file de fusion, et le premier quand la file est vide. Un temoin qui depend de l'etat de la file se perime entre deux lectures, et la garde qu'il fonde mesure alors autre chose que ce qu'elle annonce. TOUTE DESIGNATION ORDINALE EST RETIREE de cette tache : la rubrique se nomme « Prochain pas », la ligne visee se designe par CE QU'ELLE DERIVE (la liste des taches eligibles, derivee de `docs/tasks.json` seul), et le generateur porte cette designation dans son code, pas dans un commentaire. La meme interdiction vaut pour la livraison : aucun livrable de cette tache ne compte une ligne ni une rubrique par sa position.
...
**Acceptation.** DEUX LIVRABLES SORTIS DE GOV-037 LE 2026-09-16, decision de Will : ils sont MECANISABLES, mais ils ne portent pas sur la garde que GOV-037 a livree — les y laisser rendait cette tache-la INFERMABLE, qui est precisement le defaut qu'elle decrivait. (1) LE COMPOSEUR COMPARE LES MAUVAIS CHAMPS. `scripts/lot/composer.ts` detecte les collisions de lot en lisant `t.paths` SEUL : deux lots qui touchent la MEME specification ne se voient pas. Mesure du 2026-09-05 : ni `gov:tasks` ni `gov:trace` ne rougissent quand `tests{}` et `paths` divergent. Ce n'est PAS une negligence des taches, c'est LA CONVENTION — `paths` porte les fichiers de SOURCE, `tests{}` porte les SPECIFICATIONS, et les deux sont deliberement disjoints dans une large part du backlog (38 taches sur 209 mesurees le 2026-09-13 par la lentille schema ; le backlog a grossi depuis, la mesure est A REFAIRE et a IMPRIMER par le livrable, jamais a recopier). A livrer : que la detection de collision lise AUSSI `tests{}`, avec un temoin vu rougir sur deux lots qui partagent une specification et un contre-temoin vert sur deux lots disjoints — et NON une reecriture des listes de chemins. Ce qu'on fait des taches divergentes se declare dans l'acceptance de la livraison : les normaliser, ou declarer la convention et ne corriger que le composeur. (2) RIEN NE CONFRONTE LES FICHIERS D'UNE PR AUX PATHS DE SES TACHES. Mesure du 2026-09-13 : dix fichiers sous `scripts/`, `src/` et `tests/` modifies par la PR 31 ne figuraient dans les `paths` d'AUCUNE tache, dont `tests/unit/gouvernance/vues-derivees.spec.ts`, promis par DEUX `tests{}`. A livrer : une garde qui confronte la liste des fichiers TOUCHES par une PR aux `paths` declares par les taches qu'elle cite, vue rougir sur ces dix-la ou sur leur equivalent remesure, et verte sur une PR conforme. Elle vit la ou la PR est DEJA lue (`scripts/gates/gov-pr.ts`) : une seconde lecture de la forge serait une seconde source. Cette tache ne rouvre PAS GOV-037 : la garde des attributions livree par la PR 45 reste la source du jugement tache <-> paths ; ce qui s'ajoute ici est le jugement du LOT et celui de la PR.

⚠️ AJOUT DU 2026-09-16 (3) — `docs/gates.json` EST UN REGISTRE APPEND-ONLY, EXCLU DU TEST DE COLLISION. Mesure sur `docs/paths-proposes.json` : `docs/gates.json` figure dans les chemins reels de TRENTE des 68 taches de phase 0. Une disjonction stricte sur ce fichier sterilise le composeur — elle serialiserait trente lots pour un fichier dont chaque tache ajoute UNE ligne, indexee par son propre `id`. Le composeur doit donc EXCLURE NOMMEMENT ce registre du test de collision, et l'exclusion vit a UN SEUL endroit du code, avec le motif ecrit a cote : c'est un registre ou chaque PR verse SA ligne par l'outil hors depot, et dont le conflit de rebase est mecanique. L'exclusion N'EST PAS une liste extensible : ajouter un second fichier a cette liste exige un ADR, et une garde le tient. Temoin a deux faces : deux taches qui ne partagent QUE `docs/gates.json` entrent dans le meme lot ; deux taches qui partagent un second fichier en plus sont separees, et le composeur NOMME ce second fichier.

⚠️ AJOUT DU 2026-09-16 (4) — LES DEUX EXIGENCES PERDUES AU TRANSFERT DEPUIS GOV-037. Elles figuraient au cinquieme livrable de GOV-037 et ne survivent dans aucun des deux textes ; elles sont reprises ici, reformulees sur ce qu'elles gardent reellement. (4a) LES DEUX CHAMPS DIVERGENT MEME SUR DES TACHES LIVREES, ET LA DIVERGENCE SE MESURE AVANT DE SE TRANCHER. Mesure du 2026-09-16 sur les 228 taches : SEPT taches declarent dans `paths` un fichier de specification que leur `tests{}` ne revendique pas, et VINGT ET UNE revendiquent dans `tests{}` une specification absente de leurs `paths` — dont CPL-T01 et GOV-024, toutes deux `fusionnee`. Le livrable REFAIT cette mesure au jour de sa livraison, l'IMPRIME, et tranche : ou bien les deux champs sont normalises et la regle est gardee, ou bien la convention est ECRITE et c'est le composeur seul qui change. Les deux reponses sont admissibles ; l'absence de reponse ne l'est pas, parce qu'elle laisse le composeur prouver une disjonction sur un champ et le lot se faire sur l'autre. (4b) UN FICHIER DE SPECIFICATION SUIVI QUE NULLE TACHE NE REVENDIQUE EST UN TROU, ET IL S'EN MESURE CINQ. Mesure du 2026-09-16 : 37 fichiers `*.spec.ts` sont suivis, et CINQ ne figurent ni dans les `paths` ni dans les `tests{}` d'aucune tache — `adr-index-derive`, `affirmations-verifiees`, `fiches-tiers`, `integration-livrable` et `preseance`, toutes sous `tests/unit/gouvernance/`. Une specification sans tache est une garde que personne ne porte : elle survit tant qu'elle passe, et rien ne dit qui la repare quand elle casse. Le livrable NOMME les cinq, leur attribue une tache ou declare explicitement qu'elles relevent d'un porteur collectif nomme, et pose le TEMOIN A DEUX FACES : un fichier de specification neuf, ajoute sans etre revendique, fait sortir la garde en code non nul et le NOMME ; l'etat du depot une fois les cinq rattachees la fait sortir en zero, avec le compte des specifications reellement confrontees.
...
Couvre : `REQ-GOV-014`, `REQ-GOV-026`, `REQ-GOV-021`

**Acceptation.** Le pas 8 du protocole prescrit `lot:cloture --lot <id>`. Or SIX taches ont ete livrees sans lot (INT-T01b et les cinq dernieres de la phase -1) : `cloture.ts` ECRIT le champ `lot`, le rendu de resultat TIRE la liste des taches de ce meme champ, et aucun ecrivain ne pose `branch` alors que le reclassement le refuse sans. Il n'existe donc aucun chemin OUTILLE ET ECRIT pour clore une tache seule. A livrer : (1) le protocole decrit le cas « tache seule », attestee par `pr` + `branch` + sha, en un pas numerote ; (2) `lot:cloture` accepte ce cas SANS inventer de lot, et pose `branch` ; (3) le chemin outille est le SEUL : fermer la tache a la main reste refuse. TEMOIN A DEUX FACES : une tache `en_cours` livree seule, close par le chemin outille, ressort `fusionnee` avec ses trois preuves et la validation du registre sort en zero ; la meme close a la main, sans `branch`, fait sortir la garde en code non nul et la NOMME. Ordre impose : GOV-041, puis GOV-042, puis celle-ci — les trois touchent `scripts/lot/cloture.ts` et ne peuvent pas entrer dans un meme lot.
...
Couvre : `REQ-GOV-008`, `REQ-GOV-011`, `REQ-GOV-013`

**Acceptation.** Plusieurs decisions de gouvernance vivent UNIQUEMENT hors depot : la regle d'arret (un refus ne bloque que sur un ecart demontre et ouvert), qui coche les cases 3 et 8 de la definition de termine, la sortie de deux livrables de GOV-037, le choix du nom de la garde des termes interdits. Pendant ce temps la charte et le protocole disent encore qu'un refus bloque : LE DEPOT CONTREDIT LA REGLE APPLIQUEE, et c'est le lecteur suivant qui paiera. `docs/DECISIONS.md` est un registre PRODUIT, sans ecrivain : il ne convient pas. A livrer : (1) un lieu nomme pour une decision de gouvernance — un ADR de `docs/adr/partners/` — et l'ecrivain qui l'y met ; (2) les decisions du 15 et du 16 septembre ecrites, datees, avec leur portee et leur reversibilite ; (3) la charte et le protocole mis en accord avec elles, dans le meme diff. TEMOIN A DEUX FACES : une copie de travail ou la charte affirme qu'un refus bloque alors que l'ADR en vigueur dit le contraire fait sortir la garde en code non nul et NOMME les deux endroits ; l'etat du depot une fois accorde la fait sortir en zero, avec le compte des affirmations reellement confrontees.
...
Couvre : `REQ-GOV-007`, `REQ-QA-013`

**Acceptation.** PRIORITAIRE : c'est le plafond de parallelisme de toute la phase 0. La revendication d'une tache vit PAR BRANCHE dans `docs/tasks.json`, mais `gov:etat` lit les demandes de fusion ouvertes GLOBALEMENT. Chaque branche doit donc porter les N revendications, et la N+1e demande rougit les N autres — cout de synchronisation en carre du nombre de branches. AGGRAVANT MESURE : `gov:etat` s'execute AVANT lint, format, typecheck et test dans la porte A, si bien qu'un rouge venu d'une AUTRE branche laisse ces etapes en « sautees » — elles ont l'air d'avoir mesure et n'ont rien mesure. C'est le pire des deux defauts : le premier coute du temps, le second rend un vert qui ment. A livrer : (1) la revendication se DERIVE au lieu de se recopier par branche, ou le lecteur des demandes ouvertes se restreint a la branche jugee — l'un ou l'autre, jamais les deux a moitie ; (2) les etapes de mesure s'executent MEME quand `gov:etat` rougit, ou bien leur saut est rendu VISIBLE comme un echec et non comme un succes. TEMOIN A DEUX FACES : trois branches ouvertes simultanement, chacune revendiquant sa seule tache, sortent toutes les trois en zero ; une quatrieme branche qui revendique une tache deja prise fait sortir la garde en code non nul et NOMME la branche concurrente, sans rougir les trois autres.
...
Couvre : `REQ-GOV-006`, `REQ-GOV-015`, `REQ-GOV-024`

**Acceptation.** MESURE DU 2026-09-16, a refaire avant d'ecrire une ligne : `docs/PLAN-STATE.md` annonce des questions ouvertes pour Will, dont une au moins — la decision d'integration canonique, tranchee le 2026-09-03 — que le registre rend NON bloquante. `scripts/plan-state/build.ts` derive ces questions des hypotheses absentes d'une section de `docs/DECISIONS.md`, SANS passer par `registre-decisions.ts`, que le composeur et `gov:tasks` importent depuis GOV-027. C'est un TROISIEME lecteur du meme registre, et `plan-state:verifier` ne peut pas le voir : il compare la vue a son generateur, pas le generateur a la source. LE REGISTRE FAIT FOI, LA VUE EST FAUTIVE. A livrer : le generateur importe le lecteur unique, et aucun autre chemin de lecture ne subsiste dans le fichier. TEMOIN A DEUX FACES : une hypothese tranchee dans le registre fait disparaitre la question de la vue regeneree, et une hypothese reellement ouverte l'y fait apparaitre ; un second lecteur reintroduit dans le fichier fait sortir la garde du lecteur unique en code non nul et NOMME la ligne. Ordre impose, meme fichier que GOV-053 et GOV-055 : GOV-053, puis GOV-055, puis celle-ci ; jamais deux dans un meme lot.
...
Couvre : `REQ-QA-013`, `REQ-GOV-012`, `REQ-GOV-029`

**Acceptation.** SIX FAITS MESURES, et ils disent tous la meme chose : on croit la porte A armee, et rien ne le prouve. (a) La reconnaissance d'une garde appelee cherche une SOUS-CHAINE dans le texte du workflow, commentaires compris : un nom cite dans un commentaire suffit a declarer la garde appelee. (b) La population ne couvre que `scripts/gates/` : retirer l'etape de verification de la vue d'etat ne rougit RIEN. (c) Aucune garde ne refuse une etape desarmee par une condition toujours fausse. (d) La tolerance d'echec n'est pas lue sous sa forme EVALUEE, seulement sous sa forme litterale. (e) Les scripts de `package.json` des gardes ne sont pas figes : un repointage n'est rattrape que par la suite de tests ou par un travail de nuit deja rouge. (f) Les commentaires du workflow sur ces etapes sont FAUX. A livrer : UNE SEULE definition de « etape presente, active et effective », lue par une seule garde, appliquee a TOUTES les etapes de la porte A et non a une racine de repertoire. TEMOIN A DEUX FACES, un par fait : six copies de travail, chacune desarmant la porte A d'une des six facons, font sortir la garde en code non nul et NOMMENT l'etape et le mode de desarmement ; le workflow du depot la fait sortir en zero, avec le compte des etapes reellement confrontees. Groupe avec GOV-062, GOV-063 et GOV-044 : meme fichier, meme population, a serialiser.
...
Couvre : `REQ-QA-013`, `REQ-GOV-010`, `REQ-GOV-029`

**Acceptation.** LA PORTE A EST GARDEE, CE QUI LA FAIT TOURNER NE L'EST PAS. Rien de ce qui suit n'est juge aujourd'hui : un reglage du gestionnaire de paquets pose au niveau du projet, qui agit sur TOUTES les etapes alors que le temoin ne l'exerce que pour deux d'entre elles ; le repertoire de correctifs, `package.json` et le fichier de verrouillage, qui ne sont pas des chemins reserves ; les dependances corrigees et les surcharges de version, qui ne sont pas lues ; les actions tierces appelees par le workflow, y compris celles qui embarquent leur propre image ; une etape amont qui reecrit l'arbre de travail avant les gardes ; un script qui porte le nom d'une commande integree ; et l'environnement herite par le temoin qui lance l'outil. A livrer : chacun de ces sept points est juge, et le refus NOMME le point. TEMOIN A DEUX FACES : sept copies de travail, une par point, font sortir la garde en code non nul en nommant le point ; le depot la fait sortir en zero, et le vert imprime le compte des sept points reellement confrontes — jamais « tout est garde ».
...
**Acceptation.** ECHEC OUVERT, PREEXISTANT, MESURE SUR DEUX TETES DIFFERENTES : le decodage des chemins de la source unique du perimetre n'est pas injectif, si bien que deux entrees distinctes peuvent finir sous la MEME cle. La garde d'entite juge alors l'une A LA PLACE de l'autre — et c'est la garde qui protege un depot PUBLIC d'une coordonnee bancaire. Meme famille, un fichier plus loin : une table indexee par chemin dans la garde de schema. ⚠️ Le detail de la representation en cause N'EST PAS ECRIT ICI : il vit hors depot, et cette acceptance n'y renvoie par aucun chemin de machine. A livrer : (1) la source unique rend des chemins distincts pour des entrees distinctes, prouve et non suppose ; (2) toute consommatrice qui indexe par chemin refuse une collision au lieu d'ecraser en silence ; (3) A MESURER ET A ECRIRE dans l'acceptance de la livraison : la forge accepte-t-elle une telle entree au depot — si elle la refuse, la portee du correctif se reduit et c'est un fait, pas une supposition. TEMOIN A DEUX FACES, exige SOUS LINUX : un arbre de bac d'essai portant deux entrees distinctes qui se confondent aujourd'hui fait sortir la garde en code non nul et NOMME les deux chemins ; l'arbre du depot la fait sortir en zero, avec le compte des entrees reellement lues.
...
**Acceptation.** LA FRONTIERE DE MOT DES DEUX GARDES DIVERGE DE CELLE DU RENDU. Un mot que le lecteur VOIT entier peut, pour la garde, n'etre pas un mot — et inversement. Ce qui rend le defaut couteux plutot que cosmetique : la garde lexicale tient le vocabulaire des textes ADRESSES AUX APPORTEURS, celui dont un mot mal place pese dans un faisceau d'indices. Une mise en forme tapee a la main peut donc soustraire un mot aux deux gardes a la fois. ⚠️ La forme exacte qui le permet n'est PAS ecrite ici. A livrer : les deux gardes decoupent le texte sur sa forme RENDUE, et non sur sa source brute ; le decoupage vit a un seul endroit, importe par les deux. TEMOIN A DEUX FACES : un texte dont le rendu affiche un terme interdit entier, mais dont la source le coupe, fait sortir les DEUX gardes en code non nul et NOMME le fichier, la ligne et le terme ; les textes du depot les font sortir en zero, avec le compte des fichiers et des termes reellement confrontes. Contre-temoin exige : un document qui EXPLIQUE la regle garde le droit d'ecrire son contre-exemple entre accents graves — la garde ne doit pas interdire sa propre documentation, defaut deja rencontre et corrige.
...
Couvre : `REQ-GOV-001`, `REQ-GOV-032`

**Acceptation.** L'ANALYSE D'UN DOCUMENT JSON GARDE LA DERNIERE OCCURRENCE D'UNE CLE REPETEE, et aucune garde de ce depot ne lit le TEXTE du registre. Une exigence peut donc etre REMPLACEE EN SILENCE par une seconde ecriture de la meme cle, et toutes les gardes restent vertes : elles jugent l'objet obtenu, jamais le document lu. Mesure du 2026-09-16 : 355 identifiants, aucun doublon d'IDENTIFIANT — le defaut porte sur la cle d'objet, et il est donc ouvert sans etre realise. Meme famille, deja consignee : un identifiant repete dans une valeur derivee du registre. Fait joint : un registre illisible sort aujourd'hui sur une trace brute, pas sur un refus nomme. A livrer : la garde lit le TEXTE du registre et refuse toute cle ecrite deux fois, a quelque profondeur que ce soit ; un registre illisible sort par un refus NOMME. TEMOIN A DEUX FACES : un registre de bac d'essai ou une cle d'exigence est ecrite deux fois fait sortir la garde en code non nul et NOMME la cle et les deux positions ; le registre du depot la fait sortir en zero, avec le compte des cles reellement confrontees. A grouper avec la reparation de l'outil hors depot qui ecrit ce registre : c'est le meme defaut, des deux cotes de la barriere.
...
Couvre : `REQ-GOV-023`, `REQ-GOV-024`

**Acceptation.** QUATRE LECTEURS, QUATRE GRAMMAIRES, POUR UNE MEME LIGNE. Deux scripts lisent l'en-tete d'entree d'une facon, un troisieme d'une autre, un quatrieme a son propre motif et RETAPE le plancher. Les ecarts ont deja produit trois faux verdicts mesures, et le temoin d'egalite entre les quatre n'est que partiel. Fait joint : le plancher derive du guide du journal se lit aussi dans un commentaire masque du document. Le depot n'a aucun analyseur de Markdown, et c'est precisement pourquoi quatre grammaires maison ont pousse. A livrer : UNE grammaire exportee, importee par les quatre, et aucun motif local ne subsiste. ⚠️ ORDRE : cette tache passe AVANT GOV-052, ou avec elle. GOV-052 exige l'entree de journal avant la fusion ; si elle est livree d'abord, elle ecrira une CINQUIEME grammaire. TEMOIN A DEUX FACES : une entree de journal a la limite de la forme — titre rendu autrement, plancher ecrit dans un commentaire masque — est lue IDENTIQUEMENT par les quatre lecteurs, et une entree malformee les fait sortir TOUS les quatre en code non nul avec le MEME nom de refus ; le journal du depot les fait sortir en zero, avec le compte des entrees reellement lues.
...
Couvre : `REQ-GOV-021`, `REQ-GOV-003`

**Acceptation.** UNE EXEMPTION RANGEE PAR SITE EXEMPTE CE QU'ELLE N'A PAS VU. Faits mesures : une declaration de contexte posee pour un fichier vaut pour TOUT le fichier, si bien qu'une attribution NEUVE ajoutee dans le meme en-tete passe sans etre jugee ; un meme lieu porte DEUX cles d'exemption, l'une figee l'autre non ; une comparaison se fait par PREFIXE la ou elle devrait se faire par identite complete ; deux exemptions n'ont pas de second producteur ; un cas de lot sans demande de fusion n'est couvert par rien ; aucun cliquet ne tient les exemptions gabarit des taches non livrees ; et les identifiants ecrits en minuscules ou avec un trait d'union insecable ne sont pas vus du tout. A livrer : l'exemption porte sur une OCCURRENCE (fichier, ligne, identifiant complet), jamais sur un site ; chaque exemption a un second producteur ou un cliquet ; la reconnaissance des identifiants couvre la casse et les traits d'union non usuels. TEMOIN A DEUX FACES : une attribution fausse ajoutee dans un en-tete deja exempte, puis un identifiant ecrit en minuscules, font sortir la garde en code non nul et NOMMENT l'occurrence ; le depot la fait sortir en zero, avec le compte des occurrences et des exemptions reellement confrontees. Ordre : celle-ci d'abord, GOV-075 ensuite — la reparation des chemins reduit les listes figees que cette tache redefinit.
...
Couvre : `REQ-QA-001`, `REQ-QA-013`, `REQ-GOV-029`

**Acceptation.** CHAQUE INTERDIT NE JUGE QU'UNE SEULE FORME. Les formes voisines traversent : un import de module ecrit sans son prefixe, un import dynamique, une horloge systeme atteinte autrement que par son nom usuel, une ecriture sur la console atteinte par un autre chemin. C'est anterieur au lot qui a pose la configuration, et ce n'est pas une dette de style : REQ-QA-001 fait de la PURETE de `src/domain/**` la condition du calcul de commission (DM-04) et de l'horloge injectable (CPL-T13). Un domaine qui lit l'heure de la machine rend un resultat different selon le jour ou on le rejoue, et c'est un registre d'argent qu'on ne peut plus reconstituer. A FERMER AVANT LE PREMIER CODE DE DOMAINE DE LA PHASE 0. A livrer : chaque interdit juge les formes voisines de ce qu'il vise, et la liste des formes couvertes est imprimee, jamais supposee. TEMOIN A DEUX FACES : un fichier de bac d'essai sous `src/domain/` qui atteint l'horloge, le reseau et la console par CHACUNE des formes voisines fait sortir `pnpm lint` en code non nul, une fois par forme, en nommant la ligne ; le code du depot le fait sortir en zero, avec le compte des formes reellement exercees.
...
Couvre : `REQ-GOV-011`, `REQ-GOV-021`

**Acceptation.** MESURE LE 2026-09-16 SUR LA PR 46. La section « Attaque » n'est exigee que par deux declencheurs, et les DEUX manquent le cas le plus courant de la phase 0. (1) LE DECLENCHEUR PAR ZONE EST MORT : il compare le debut du chemin d'un fichier a quatre prefixes de repertoire qui ne correspondent a AUCUN chemin du depot — mesure : ZERO fichier suivi commence par l'un d'eux, et zero fichier suivi les contient meme au milieu de son chemin. Il n'a donc jamais rien declenche, et il ne declenchera rien quand les repertoires reels apparaitront, parce que ceux-ci vivront sous `src/`. (2) LE DECLENCHEUR PAR TACHE resout les taches de la demande de fusion par le numero qu'elles portent ou par l'identifiant du titre. Une demande qui REECRIT LA PROSE d'une tache sensible — son texte d'acceptation, son titre, ses champs dans le registre — ne « porte » donc pas cette tache : la tache pointe une autre demande, et le titre nomme autre chose. Le scenario d'attaque n'est exige par PERSONNE. C'est exactement ce qui s'est passe sur la PR 46 avec une tache marquee argent : le contenu etait propre, et aucune garde ne l'exigeait — un vert qui ne mesurait rien. A livrer : (1) le declencheur par zone compare des chemins REELS, derives du disque, et non des prefixes tapes ; il est ecrit a un seul endroit et imprime ce qu'il a confronte ; (2) une demande de fusion qui MODIFIE l'entree de registre d'une tache sensible exige le scenario au meme titre qu'une demande qui la porte ; (3) la sortie NOMME la tache et le motif du declenchement. TEMOIN A DEUX FACES : une demande de fusion qui ne touche que la prose d'une tache marquee argent, sans section d'attaque, fait sortir la garde en code non nul et NOMME la tache et son marquage ; la meme avec sa section remplie sort en zero. Et un fichier place sous le chemin reel d'une zone sensible declenche l'exigence, alors qu'il ne la declenche pas aujourd'hui.
...
Couvre : `REQ-GOV-032`, `REQ-GOV-026`

**Acceptation.** CONSTAT MESURE PAR LA REPETITION A BLANC DU 2026-09-16, et il corrige une premisse fausse de ma part. J'avais demande de verifier que GOV-050 et GOV-051 restent NOMMEES dans `docs/PLAN-STATE.md` apres le basculement en phase 0. Elles ne le sont NI AVANT NI APRES — et ce n'est pas une regression : `plan-state:build` tronque la rubrique des taches a faire a DOUZE identifiants suivis de points de suspension. Les deux taches sont nommees dans `docs/TASKS.md`, `docs/TRACABILITE.md` et `docs/REQUIREMENTS.md` ; la vue d'etat, elle, en montre douze sur soixante-dix-huit. CE QUI REND CELA COUTEUX PLUTOT QUE COSMETIQUE : la vue d'etat est le document qu'on lit pour savoir ou en est le projet, et une troncature muette y fabrique une impression de petit reste. Aucun lecteur ne peut distinguer « douze taches a faire » de « douze taches a faire affichees sur soixante-dix-huit », et c'est la meme famille que le compte tape qui ne tient pas — la quatrieme de ce depot. A livrer : (1) toute liste tronquee de la vue imprime, a l'endroit meme de la troncature, le COMPTE TOTAL et le compte affiche, derives, jamais tapes ; (2) la rubrique nomme la vue ou la liste COMPLETE se lit, pour que le lecteur ait un chemin et non une frustration ; (3) le choix du plafond (douze) est ecrit avec son motif a cote du code qui l'applique. TEMOIN A DEUX FACES : une vue regeneree sur un registre portant plus de taches a faire que le plafond imprime le total et le compte affiche, et la garde sort en zero ; la meme vue ou le total est retire, ou bien recopie a la main au lieu d'etre derive, fait sortir la garde en code non nul et NOMME la rubrique tronquee. CE QUE CETTE TACHE NE FAIT PAS : elle ne releve pas le plafond. Douze est peut-etre le bon nombre ; ce qui n'est pas bon, c'est qu'il se taise.
...
Couvre : `REQ-GOV-021`, `REQ-GOV-024`

**Acceptation.** PETITE TACHE, FAMILLE CONNUE. Dans `scripts/gates/gov-attributions.ts`, le registre des dettes de reciprocite porte, pour l'entree de la garde de derivation, une raison qui dit que le registre « ecrit noir sur blanc » qu'une certaine re-attribution viderait un temoin, en NOMMANT la tache. Le lot preparatoire a amende cette prose du registre : la tache nommee n'y figure plus. L'affirmation du code est donc DEVENUE FAUSSE, sans qu'aucune ligne du code ait bouge. AUCUN ROUGE NE LA SIGNALE, et c'est precisement ce qui la rend dangereuse : les raisons de ce registre ne sont jugees que sur leur LONGUEUR — vingt caracteres suffisent, la verite n'est pas mesuree. C'est mot pour mot ce que `gov:sonde` interdit de garder en silence pour les affirmations sur l'autre depot, applique cette fois a une affirmation du depot sur LUI-MEME. A livrer : (1) la raison ne nomme plus une tache qu'elle ne controle pas — elle DERIVE la designation du registre, ou elle parle de « la tache successeur » sans la nommer, ce qui reste vrai quelle qu'elle soit (RM-01) ; (2) UNE GARDE, et c'est elle qui vaut le detour : toute raison de ce registre qui cite un identifiant de tache est confrontee au registre — si l'identifiant ne s'y trouve plus a l'endroit annonce, la garde ROUGIT. Une raison ne doit pas pouvoir devenir fausse en silence deux fois. TEMOIN A DEUX FACES : une raison qui nomme une tache absente de l'endroit qu'elle designe fait sortir la garde en code non nul et NOMME l'entree et l'identifiant ; le registre du depot la fait sortir en zero, avec le compte des raisons reellement confrontees. ⚠️ Elle partage `scripts/gates/gov-attributions.ts` avec GOV-073, GOV-074 et GOV-075 : elle ne peut pas entrer dans leur lot, et elle passe APRES GOV-074, qui redefinit les listes que cette raison commente.
...
Couvre : `REQ-GOV-005`, `REQ-GOV-012`

**Acceptation.** FAIT MESURE TROIS FOIS LE 2026-09-16, SUR UN BAC VIERGE — donc a la tete du depot, et non introduit par un lot. `scripts/gates/gov-trace.ts` resout les titres de tests en lancant un PROCESSUS ENFANT qui enumere les cas de la suite. Selon le LANCEUR par lequel la garde est elle-meme invoquee, cet enfant rend soit la liste, soit UNE LISTE VIDE AVEC LE CODE ZERO. Et le code lit la seconde reponse comme la premiere : « resolu, aucun titre ne correspond » — d'ou 53 fautes `test_promis_absent` FABRIQUEES, sur un depot ou rien ne manque.

| invocation | verdict |
| --- | --- |
| par le binaire du transpileur, chemin explicite | 53 `test_promis_absent` |
| idem, avec la variable d'environnement equivalente | 53 |
| par le lanceur de paquets | matrice coherente, sortie zero |

➡️ CONSEQUENCE IMMEDIATE, ET ELLE CORRIGE UNE CONCLUSION : `gov:trace` est VERTE a la tete. La lecture « 53 fautes deja presentes » etait une lecture de l'outil, pas du depot.

POURQUOI C'EST PIRE QU'UN ECHEC OUVERT. La famille connue est « un predicat ouvert echoue ouvert » : faute de savoir, la garde laisse passer. Ici la garde echoue FERME — faute de savoir, elle ACCUSE. Un echec ouvert rend un vert qu'on sait suspect ; celui-ci rend un rouge qui A L'AIR D'UN RESULTAT, avec 53 lignes nominatives, et qui envoie le lecteur suivant corriger 53 choses qui n'ont rien. C'est le cout le plus eleve de toute la famille : il ne fait pas perdre une mesure, il fait perdre une journee.

A LIVRER. (1) UNE LISTE VIDE RENDUE AVEC LE CODE ZERO EST UN REFUS NOMME — `titres_non_resolus` — et JAMAIS la preuve que les titres promis n'existent pas. La garde distingue trois etats, et les nomme : l'enumeration a rendu des titres ; elle a rendu zero titre alors que la suite en contient (refus) ; elle a echoue (refus). Aujourd'hui les trois se lisent pareil. (2) LA GARDE NE DEPEND PAS DU LANCEUR QUI L'INVOQUE — ou, si elle en depend, elle le DIT et le VERIFIE au lieu de le supposer : elle controle que l'enumeration a rendu un nombre de titres COHERENT avec ce que le disque contient, et refuse quand les deux divergent. Un plancher derive, jamais un plancher tape. (3) TEMOIN A DEUX FACES, ET LES DEUX FACES SE JOUENT : sous un enumerateur qui rend la liste, la garde voit les titres et sort en zero ; sous un enumerateur qui rend le VIDE avec le code zero, elle REFUSE en nommant `titres_non_resolus` — elle n'accuse aucun test. Un temoin qui ne joue que la face qui marche ne mesure rien, et c'est exactement pourquoi ce defaut a survecu jusqu'a la repetition finale. (4) Le vert imprime le compte des titres REELLEMENT enumeres, jamais la longueur d'une liste declaree.

⚠️ ELLE PARTAGE `scripts/gates/gov-trace.ts` AVEC GOV-043 ET QA-T03 : elle ne peut pas entrer dans leur lot, et elle passe APRES GOV-043, qui redefinit ce que cette garde declare ne pas couvrir.
...
Couvre : `REQ-GOV-012`, `REQ-QA-013`

**Acceptation.** LE MIROIR EXACT DE GOV-044, ET IL MORD PLUS FORT. GOV-044 dit qu'une garde ABSENTE DU REGISTRE s'exempte elle-meme. Celle-ci dit l'inverse : une entree DU REGISTRE dont le script est absent du DISQUE s'exempte elle aussi — et par la meme ligne. `gardesJugeables()` retient une entree seulement si son script figure parmi les fichiers suivis : ce qui n'est pas la n'est pas juge, et rien ne le DIT. MESURE DERIVEE LE 2026-09-16, et je l'ecris avec son predicat pour qu'elle se re-derive au lieu de se recopier — predicat : entree de `docs/gates.json` dont le `script` commence par `scripts/gates/` et ne figure pas dans les fichiers suivis. AVANT le lot preparatoire : 36 entrees sur 62. APRES : 30, parce que le lot re-prefixe six scripts vers l'autre depot — et ces six-la sortent alors de la famille SANS que personne n'ait rien juge, ce qui est precisement le probleme. TROIS SOUS-FAMILLES SE CACHENT SOUS UN SEUL SILENCE : le script vit dans l'autre depot ; le script n'est pas encore ecrit, parce que la garde est promise a une phase future ; le script n'existera jamais sous ce nom, parce que l'entree est fautive — c'est le cas de GOV-044 lui-meme, dont le registre nommait `scripts/gates/gov-gates.ts`, un fichier qui n'a jamais existe. Aujourd'hui les trois rendent le meme silence, et seule la troisieme est une faute. A LIVRER. (1) Les trois sous-familles sont DISTINGUEES et NOMMEES : une entree qui designe l'autre depot est declaree telle et sortie du perimetre AVEC SON MOTIF ; une entree promise a une phase future est declaree telle, avec sa phase ; une entree de la phase courante dont le script est introuvable est un REFUS. (2) Le vert IMPRIME le compte des entrees reellement jugees et le compte de celles qui sont sorties, par sous-famille — « aucune garde ecrite jamais appelee » sur un perimetre de zero entree se lit comme une absence prouvee, et c'est la quatrieme fois que ce depot rencontre cette forme. (3) TEMOIN A DEUX FACES : une entree de la phase courante dont on supprime le script fait sortir la garde en code non nul et NOMME l'entree et le chemin manquant ; le registre du depot la fait sortir en zero, avec les trois comptes imprimes. ⚠️ Elle partage `scripts/gates/gov-conventions.ts` avec GOV-044, GOV-051 et GOV-061 : chaine de quatre, jamais deux dans un meme lot, et elle passe APRES GOV-044, qui derive le perimetre du disque.
...
**Acceptation.** MEME FAMILLE QUE LES SPECIFICATIONS ORPHELINES, DE L'AUTRE COTE. GOV-056 mesure cinq fichiers de specification suivis que nulle tache ne revendique. La mesure symetrique, faite le 2026-09-16 sur les scripts, en donne NEUF : `gates-derivees.ts`, `gates-prouvees.ts`, `gh-sur.js`, `git-push-sur.js`, `gov-autonomie.ts`, `gov-hypotheses.ts`, `gov-preseance.ts`, `gov-publication.ts` et `hook-env.js` — tous sous `scripts/gates/`, tous suivis, aucun dans les `paths` d'une tache. Et le compte n'est pas la ou on le croit : sur 29 fichiers suivis de ce repertoire, NEUF n'ont pas de porteur, soit pres d'un sur trois. UN SCRIPT SANS TACHE EST UNE GARDE QUE PERSONNE NE PORTE : elle survit tant qu'elle passe, et le jour ou elle casse, rien ne dit qui la repare ni contre quelle exigence elle a ete ecrite. Deux d'entre eux sont des barrieres d'execution (`gh-sur.js`, `git-push-sur.js`) et un troisieme est le hook d'environnement : ce sont les fichiers dont la disparition serait la MOINS visible et la plus couteuse. A LIVRER. (1) La liste des scripts suivis sans porteur est DERIVEE du disque, imprimee avec son compte, et confrontee au registre — jamais tapee. (2) Chacun des neuf recoit une tache, ou bien une declaration EXPLICITE de porteur collectif qui dit lequel et pourquoi ; le silence n'est plus une reponse admise. (3) TEMOIN A DEUX FACES : un script de garde neuf, suivi et revendique par aucune tache, fait sortir la garde en code non nul et le NOMME ; l'etat du depot une fois les neuf rattaches la fait sortir en zero, avec le compte des scripts reellement confrontes. ⚠️ Elle partage `scripts/gates/gov-attributions.ts` avec GOV-073, GOV-074, GOV-075 et GOV-081 : chaine de cinq, a serialiser, et elle passe APRES GOV-074.
...
Couvre : `REQ-QA-014`, `REQ-GOV-032`

**Acceptation.** LE DEFAUT LE PLUS RETORS DE LA SERIE, PARCE QUE SA VICTIME CROIT AVOIR OBEI. Quand la matrice de tracabilite denonce une attribution — une tache promet un test qui ne la cite pas — le message conduit naturellement au geste de re-rendre la vue. Or RE-RENDRE ECRIT L'ATTRIBUTION FAUSSE DANS LA VUE, et le controle suivant compare la vue a son generateur : les deux concordent, le rouge s'eteint. LE GESTE QUE LE ROUGE PRESCRIT A DONC POUR EFFET DE RENDRE VRAIE L'ERREUR QU'IL DENONCAIT. Personne n'a triche ; le circuit est simplement ferme sur lui-meme. C'est la meme forme que « comparer un generateur a lui-meme ne voit pas ce qu'il a cesse » (GOV-055), un cran plus loin : ici le generateur ne se contente pas de ne pas voir, il ENTERINE. A LIVRER. (1) LES DEUX QUESTIONS SONT SEPAREES, et le code ne les melange plus : « la vue est-elle a jour de sa source » et « la source dit-elle vrai du disque » sont deux controles, deux sorties, deux messages. Rendre la vue ne repond JAMAIS a la seconde. (2) Le message qui denonce une attribution ne prescrit PAS de re-rendre : il nomme la tache, le test promis, et dit que le remede est de corriger l'un des deux — jamais de regenerer. Un message qui propose le mauvais geste est un piege poli. (3) TEMOIN A DEUX FACES : une tache qui promet un test dont le titre ne la cite pas fait sortir la garde en code non nul AVANT et APRES un rendu complet de la vue — c'est la face qui manque aujourd'hui, et c'est elle qui prouve le defaut ; le registre du depot la fait sortir en zero, avec le compte des promesses reellement confrontees. ⚠️ Elle partage `scripts/gates/gov-trace.ts` avec GOV-043, QA-T03 et GOV-082 : chaine de quatre, et elle passe APRES GOV-082, sans laquelle le controle ne sait meme pas si sa liste de titres est vraie.
...
Couvre : `REQ-GOV-021`, `REQ-GOV-007`

**Acceptation.** MESURE QUI OUVRE LA TACHE, a rejouer et non a recopier. Le 2026-09-23, `pnpm gov:pr -- --pr 114` rougit sur la famille `fichier_hors_paths_des_taches` en nommant douze fichiers de code « qu aucune de ses taches ne declare », et la PR #114 n est donc pas fusionnable. Or elle porte SIX taches (GOV-046, GOV-048, GOV-076, GOV-078, GOV-082, GOV-086) sur une branche, un commit par tache — la forme NORMALE que `docs/CONVENTIONS.md` §5 et `partners/ADR-0007` fixent : un lot, une branche, une PR. Neuf des douze fichiers nommes sont declares, par les CINQ AUTRES taches du lot. CAUSE MESUREE : `tachesDeLaPr()` (`scripts/lot/revues.ts`, appelee a deux endroits de `scripts/gates/gov-pr.ts`) apparie une tache a une PR sur `t.pr === <numero>` OU sur l identifiant du TITRE. Le titre d une PR de lot ne peut nommer qu UNE tache, et `t.pr` n est ecrit que par `pnpm lot:cloture`, dont l invariant exige `fusion.atterri === true` — donc APRES la fusion. Les cinq autres taches ne resolvent ni par l un ni par l autre. PORTEE : tant que ce defaut existe, AUCUNE PR de lot n est fusionnable, et le lot est la seule forme qui divise le cout du protocole de relecture. ⛔ LE CONTOURNEMENT QUE LA GARDE PROPOSE EST UN FAUX, et c est ce qui rend le defaut bloquant plutot que genant : son message prescrit d ajouter les douze chemins aux `paths` de la tache du TITRE par `hors-depot/ajouter-path.mjs`. Ce serait ecrire que cette tache touche des fichiers qui appartiennent a cinq autres taches — et la disjonction des lots se calcule sur ces memes `paths`. Le lead du lot a refuse de le faire, et il a eu raison. A LIVRER. (1) UN CHAMP `Lot:` dans le gabarit de PR, section Identite, a cote d `Auteur:`, `Relecteur:` et `Couvre:` : il porte les identifiants de tache separes par des VIRGULES, ou reste VIDE pour une PR a une seule tache. (2) `CHAMPS` de `gov-pr.ts` le garde comme les autres, pour que `champ_gabarit_absent` rougisse s il disparait du gabarit — mais il n est JAMAIS exige rempli dans le corps d une PR : l exiger rendrait la garde insatisfiable pour la tres grande majorite des PR. (3) `tachesDeLaPr()` resout desormais par l UNION DES TROIS : `t.pr === <numero>`, l identifiant du titre, les identifiants declares dans `Lot:`. La monotonie que son docblock nomme est preservee : un renseignement de plus ne peut que faire GROSSIR l ensemble, donc la garde obtient toujours un sur-ensemble de ce que le composeur du corps voit. (4) LE NIVEAU DE CONFIANCE NE CHANGE PAS, et c est l argument que l ADR doit porter : le TITRE est deja ecrit par l auteur de la PR et la garde le croit depuis le premier jour ; `Lot:` est exactement aussi fiable — meme auteur, meme corps, meme absence de contreseing — et il est EXPLICITE et AUDITABLE la ou le titre est implicite. (5) QUATRE REFUS, CHACUN AVEC SON TEMOIN VU ROUGIR, et un identifiant refuse n elargit RIEN (sens de defaillance ferme) : un identifiant inconnu du registre (`lot_tache_inconnue`) ; un identifiant deja livre, une PR ne rouvrant pas une tache livree (`lot_tache_livree`) ; un identifiant qui porte un `pr` DIFFERENT (`deux_pr_meme_tache` — le NOM que `gov:etat` donne deja a cette regle, REPRIS et non double, `partners/ADR-0011` : les deux gardes l observent sur deux populations disjointes, les TITRES des PR ouvertes de la forge d un cote, le champ `pr` du registre confronte a la seule PR jugee de l autre) ; un champ MAL FORME — separateur inattendu, virgule en trop (`lot_mal_forme`), jamais une liste vide silencieuse, ce qui est precisement GOV-082. (6) DEUX CONTRE-TEMOINS VERTS AU MOINS : `Lot:` vide ou absent laisse le comportement INCHANGE — c est le cas d aujourd hui et de la tres grande majorite des PR ; et la MEME PR de lot, champ rempli, sort en zero la ou elle rougissait sans lui — sans cette seconde face on ne saurait pas si le champ RESOUT les taches ou s il a seulement fait taire la famille. (7) Le message de `fichier_hors_paths_des_taches` cesse de prescrire le remede FAUX sur une PR de lot : il nomme `Lot:` avant `ajouter-path.mjs`. (8) `risqueDeLaPr()` recoit la meme union : sans elle, une tache `sensible` ou `schema: true` portee par une autre tache du lot que celle du titre n exigerait ni section Attaque ni approbation bloquante de l architecte.
...
**Acceptation.** DECISION DE L ORCHESTRATEUR SUR DELEGATION DE WILL (aller beaucoup plus vite), puis DECISION DE WILL DU 2026-09-26 sur les lentilles et la mutation. MESURE QUI L OUVRE : environ 9 PR en 12 h, 3 a 4 tours de relecture par PR, pres de la moitie des tours ne corrigent aucun defaut : chaque fusion met les autres PR en conflit sur les vues derivees commitees et sur le cliquet litteral des sorties non nulles ; fusionner main perime tous les accords meme quand le diff propre a la PR est identique ; la porte A rougit apres les relectures. LIVRABLES. (1) VUES D ETAT HORS DES PR, premier pas sur `docs/PLAN-STATE.md` et `docs/TRACABILITE.md` : en PR, la porte A les regenere sur l arbre fusionne avant toute garde qui les lit (`pnpm vues:fusion`), et sur `push` de main elle les regenere et les commite quand un verificateur strict rougit ou que le commit de tete ne porte pas l etat vivant, puis pousse par un jeton de bot nomme ; sans jeton elle rougit en le nommant. Les verificateurs stricts restent dans la porte A (REQ-GOV-032). (2) CLIQUET CALCULE : le total des sorties non nulles declarees n est plus un litteral ; une declaration presente sur la base ne disparait qu avec son fichier et son total ne baisse pas, une sortie ajoutee non declaree rougit toujours. (3) UN ACCORD SURVIT A UNE FUSION DE MAIN SANS CHANGEMENT DE PATCH : empreinte `git patch-id --stable` du diff propre a la PR (depuis la base de fusion avec main), hors vues d etat ; temoins a deux faces sur un vrai depot jetable : fusion de main sans changement du patch, accord conserve ; une ligne changee dans un fichier de la PR, perime ; resolution de conflit qui modifie une ligne de la PR, perime ; la lentille exactitude suit la meme regle. (4) `pnpm pre-gate` : les etapes rapides de la porte A lues dans ci.yml, sans la suite de tests ni les navigateurs, documente dans PROTOCOLE-FUSION et la charte. (5) DEUX LENTILLES PARTOUT, exactitude et securite, le refus securite reste un veto ; l avis signe A02 schema reste exige des qu une PR touche au schema ; decision datee au registre, ADR qui remplace partners/ADR-0021. (6) MUTATION PAR STRYKER : la lentille mutation n est plus une revue d agent ; `pnpm mutation:pr` mute en bac a sable les fichiers de src/domain, src/server et des scripts de garde touches par la PR, nomme chaque survivant fichier:ligne et rougit sous le seuil ; appele dans la porte A.
...
### UX-P0-01b — Libelles d'etats d'attribution dans la micro-copy, une fois les etats livres

`0.25 j` · zone `espace` · sensible : attribution · depend de `UX-P0-01`, `DM-07`

Couvre : `REQ-UX-002`, `REQ-UX-003`
...
### GOV-080 — Une mention de gate devient irrefermable des qu'elle traverse la frontiere des deux depots

`0.25 j` · zone `gouvernance` · depend de `DM-03-P`

Couvre : `REQ-GOV-021`, `REQ-DM-014`
