...
| Qu’est-ce qui est en vol ? | 1. #88 (rien) · 2. #90 (rien) · 3. #99 (rien) · 4. #100 (rien) · 5. #82 (un contrôle requis rouge ou une revue manquante) · 6. #86 (un contrôle requis rouge ou une revue manquante) · 7. #91 (un contrôle requis rouge ou une revue manquante) · 8. #92 (un contrôle requis rouge ou une revue manquante) · 9. #93 (un contrôle requis rouge ou une revue manquante) · 10. #102 (un contrôle requis rouge ou une revue manquante) |
...
| Le prochain pas | fusionner #88, puis DM-02 — Gates de schéma : enums, centimes, index partiels, migrations additives (chemin critique) |
...
| 3 | #99 — feat(GOV-047): pnpm prevol existe enfin, derive du job gate-a et non de la chaine gov:check | `t/gov-047` | rien — fusionnable maintenant |
| 4 | #100 — fix(GOV-059): la porte A ne tourne plus sur une PR fusionnee, gov:pr nomme la bonne cause, et ADR 0017 | `t/gov-059-suite` | rien — fusionnable maintenant |
| 5 | #82 — feat(QA-T07): gate securite semgrep, regles maison vues rougir, image epinglee | `t/qa-t07` | un contrôle requis rouge ou une revue manquante |
| 6 | #86 — feat(QA-T02): harnais d'integration testcontainers, environnement construit, Redis reel du script de SEC-10 | `t/qa-t02` | un contrôle requis rouge ou une revue manquante |
...
| 10 | #102 — docs(GOV-063): ADR 0017 tranche l'homonymie, le nom gov:check est retire des deux cotes | `t/gov-check-homonymie` | un contrôle requis rouge ou une revue manquante |
...
`docs/adr/0011-une-seule-implementation-des-listes-d-etats.md` — partners/ADR-0011 — Les listes d'états occupants ont UNE implémentation, et son discriminant est la couverture · `docs/adr/0017-un-nom-une-garde-gov-check-retire-des-deux-cotes.md` — partners/ADR-0017 — Un nom, une garde : `gov:check` est retiré des deux côtés
