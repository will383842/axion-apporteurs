...
| HYP-W15-FILLEULS-VUE | « Mes filleuls » : ce que le parrain voit (W15, point 1) | La liste des filleuls **directs** porte, pour chacun, le **prénom**, l'**initiale du nom** et l'**état du contrat**, par une **règle fermée** dérivée de l'**ensemble des versions** du contrat du filleul (`StatutContrat`, REQ-DM-013), jamais d'une version « courante » : une version `signe` et aucune `resilie` → « signé » — un **avenant** envoyé et jamais signé (REQ-CPL-028) ne change **rien**, un avenant signé non plus ; aucune `signe` et au moins une `resilie` → le filleul **sort de la liste** ; toutes les versions `envoye` → « en signature » ; aucune version → absent ; toute autre combinaison → **échec fermé**, filleul absent et écart journalisé en console, jamais un libellé par défaut. Le **statut de l'apporteur filleul** (`StatutApporteur`, dont `pret_a_signer`, `suspendu`, `resilie`), son gel, ses anomalies et sa sincérité **n'entrent jamais dans le calcul ni dans le DTO** : une suspension pour fraude présumée ne change rien à ce que voit le parrain. Jamais l'e-mail, le téléphone, un identifiant interne, `actif`/`dormant`, un compteur ni une date d'activité ; **aucun montant par filleul** (REQ-UX-041, REQ-UX-006 amendée). ⚠️ **Dette nommée, non tranchée** : l'échéance des 12 mois que `/filleuls` affiche (UX-P2-04), agrégée sur un parrain qui n'a **qu'un** filleul dans sa fenêtre, révèle la date de signature de ce filleul ; la masquer (par exemple, aucune échéance affichée tant qu'un seul filleul est dans sa fenêtre) est une décision de Will, à prendre avant UX-P2-04 | paramètre | 2 | — | — |
| HYP-W15-ART-4-6 | Art. 4.6 al. 6 du gabarit face à la liste des filleuls (W15, point 1) | **Le gabarit est AMENDÉ** (décision de Will du 2026-09-25, « fais selon tes recommandations ») : l'al. 6 autorise au parrain la **liste de ses filleuls directs**, réduite au prénom, à l'initiale du nom et à l'état du contrat, « en signature » ou « signé » **seulement** (règle fermée de `HYP-W15-FILLEULS-VUE`, sur l'ensemble des versions : un avenant en cours de signature ne change rien) ; une relation résiliée fait **sortir** le filleul de la liste — le parrain n'apprend pas la fin d'une relation — et aucune mesure prise à l'égard du filleul (suspension, vérification) ne lui est portée. Aucun montant par filleul, aucune donnée d'activité, aucun second niveau — inchangé. Rédaction portée par JUR-T01 ; texte définitif soumis à la relecture de Will avant signature (JUR-T01b) | **avenant** | 2 | **premier DocuSeal** | **2026-09-25** |
