...
| HYP-W15-FILLEULS-VUE | « Mes filleuls » : ce que le parrain voit (W15, point 1) | La liste des filleuls **directs** porte, pour chacun, le **prénom**, l'**initiale du nom** et l'**état du contrat** (`signature_en_cours`, `signe`, `termine`) ; jamais l'e-mail, le téléphone, un identifiant interne, `actif`/`dormant`, un compteur ni une date d'activité ; **aucun montant par filleul** (REQ-UX-041, REQ-UX-006 amendée) | paramètre | 2 | — | — |
| HYP-W15-ART-4-6 | Lecture de l'art. 4.6 al. 6 du gabarit face à la liste des filleuls (W15, point 1) | **Aucune modification du gabarit** : l'al. 6 refuse au parrain toute « donnée relative à l'activité du filleul autre que le montant du parrainage » ; le prénom, l'initiale et l'état du contrat sont lus comme des données d'**identité et de relation contractuelle**, pas d'activité. ⚠️ Lecture non éprouvée : l'état `termine` révèle la fin d'une relation. Deux issues sûres, au choix de Will : écrire à l'al. 6 « Le parrain voit la liste de ses filleuls (prénom, initiale du nom, état du contrat) », ou retirer l'état de la liste | **avenant** | 2 | **premier DocuSeal** | — |
| HYP-W15-PARRAIN-A-DATE | Changement de parrain : effet et fondement contractuel (W15, point 2) | **Effet à la date de l'action, sans rétroactivité** : les lignes `parrainage` déjà nées restent au parrain d'origine ; une ligne du filleul acquise après la date d'effet profite au nouveau parrain ; une `reprise` suit le parrain de la ligne qu'elle reprend ; la fenêtre reste `contratFilleulSigneAt + 12 mois` (REQ-DM-044). ⚠️ L'art. 4.6 al. 1 ouvre au parrain qui a **présenté** le filleul un droit sur ses commissions : lui retirer les lignes futures n'a **aucune base écrite** dans le gabarit. Deux issues, au choix de Will : écrire à l'art. 4.6 que la Société peut corriger, sur motif, le rattachement d'un filleul (code erroné, présentation contestée), ou n'autoriser le changement qu'avec l'accord écrit du parrain d'origine | **avenant** | 2 | **premier DocuSeal** | — |
