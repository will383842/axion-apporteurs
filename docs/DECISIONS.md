...
| HYP-W15-FILLEULS-VUE | « Mes filleuls » : ce que le parrain voit (W15, point 1) | La liste des filleuls **directs** porte, pour chacun, le **prénom**, l'**initiale du nom** et l'**état du contrat**, par une **table fermée** dérivée UNIQUEMENT du `StatutContrat` du contrat **courant** (REQ-DM-013) : `envoye` → « en signature » ; `signe` et `remplace` → « signé » (un contrat remplacé a une version courante, c'est elle qui compte) ; `resilie` → le filleul **sort de la liste** ; sans contrat courant, absent ; valeur hors table → **échec fermé**, filleul absent et écart journalisé en console, jamais un libellé par défaut. Le **statut de l'apporteur filleul** (`StatutApporteur`, dont `pret_a_signer`, `suspendu`, `resilie`), son gel, ses anomalies et sa sincérité **n'entrent jamais dans le calcul ni dans le DTO** : une suspension pour fraude présumée ne change rien à ce que voit le parrain. Jamais l'e-mail, le téléphone, un identifiant interne, `actif`/`dormant`, un compteur ni une date d'activité ; **aucun montant par filleul** (REQ-UX-041, REQ-UX-006 amendée) | paramètre | 2 | — | — |
| HYP-W15-ART-4-6 | Art. 4.6 al. 6 du gabarit face à la liste des filleuls (W15, point 1) | **Le gabarit est AMENDÉ** (décision de Will du 2026-09-25, « fais selon tes recommandations ») : l'al. 6 autorise au parrain la **liste de ses filleuls directs**, réduite au prénom, à l'initiale du nom et à l'état du contrat, « en signature » ou « signé » **seulement** (table fermée de `HYP-W15-FILLEULS-VUE`) ; un contrat `resilie` fait **sortir** le filleul de la liste — le parrain n'apprend pas la fin d'une relation — et aucune mesure prise à l'égard du filleul (suspension, vérification) ne lui est portée. Aucun montant par filleul, aucune donnée d'activité, aucun second niveau — inchangé. Rédaction portée par JUR-T01 ; texte définitif soumis à la relecture de Will avant signature (JUR-T01b) | **avenant** | 2 | **premier DocuSeal** | **2026-09-25** |
